...
use prover::parse_input::*;
use prover::utils::CBytes;

pub fn build_machine(validation_entry_file: PathBuf, machines: PathBuf) -> eyre::Result<Machine> {
    ...
    let end_parent_chain_block_hash = Bytes32::try_from(data.end_parent_chain_block_hash)?;
    mach.set_preimage_resolver(preimage_resolver);
    mach.set_global_state(start_state);
    mach.set_end_parent_chain_block_hash(end_parent_chain_block_hash);
    Ok(mach)
}
