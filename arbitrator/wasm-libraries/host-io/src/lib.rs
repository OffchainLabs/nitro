...
extern "C" {
    ...
    pub fn wavm_get_end_mel_root(ptr: *mut u8);
...
/// Reads 32-bytes of the ending MEL root.
#[no_mangle]
pub unsafe extern "C" fn wavmio__getEndMELRoot(out_ptr: GuestPtr) {
    let mut our_buf = MemoryLeaf([0u8; 32]);
    let our_ptr = our_buf.as_mut_ptr();
    assert_eq!(our_ptr as usize % 32, 0);
    wavm_get_end_mel_root(our_ptr);
    STATIC_MEM.write_slice(out_ptr, &our_buf[..32]);
}
