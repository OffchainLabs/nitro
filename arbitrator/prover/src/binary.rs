...
use wasmparser::{
    BinaryReader, Data, Element, ExternalKind, MemoryType, Name, NameSectionReader, Naming,
    Operator, Parser, Payload, TableType, TypeRef, ValType, Validator, WasmFeatures,
};
...
pub fn parse<'a>(input: &'a [u8], path: &'_ Path) -> Result<WasmBinary<'a>> {
    ...
    features.set(WasmFeatures::COMPONENT_MODEL_VALUES, false);
    features.set(WasmFeatures::COMPONENT_MODEL_NESTED_NAMES, false);
