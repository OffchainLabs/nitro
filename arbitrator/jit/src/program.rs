...
pub fn start_program(
    ...
    if genv.wenv.threads.len() as u32 != module || module == 0 {
        return Escape::hostio(format!("got request for thread {module} but len is {}", genv.wenv.threads.len()));
    }
...
pub fn activate(mut env: WasmEnvMut,
    wasm_ptr: Uptr,
    wasm_size: u32,
    pages_ptr: Uptr,
    version: u16,
...
///
pub fn new_program(mut env: WasmEnvMut,
    ...
    let evm_data: EvmData = unsafe {
        *Box::from_raw(evm_data_box as *mut EvmData)
    };
    let config: JitConfig = unsafe {
        *Box::from_raw(config_box as *mut JitConfig)
    };
    ...
    let Some(module) = genv.wenv.module_asms.get(&compiled_hash).cloned() else {
        return Err(Escape::Failure(format!("module hash {:?} not found in {:?}",
        compiled_hash, genv.wenv.module_asms.keys())));
    };

    let cothread = exec_wasm(module, calldata, 
        config.compile, config.stylus, evm_data, ink).unwrap();
...
pub fn set_response(mut env: WasmEnvMut,
    id: u32,
    gas: u64,
    reponse_ptr: Uptr,
    response_len: u32,
...
pub fn create_stylus_config(mut _env: WasmEnvMut,
    version: u16,
    max_depth: u32,
    ink_price: u32,
    debug: u32,
...
///
pub fn create_evm_data(mut env: WasmEnvMut,
