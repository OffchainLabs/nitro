...
impl<'s> CallerEnv<'s> {
    ...
    pub fn caller_read_u8(&self, ptr: u32) -> u8 {
        let ptr: WasmPtr<u8> = WasmPtr::new(ptr);
        ptr.deref(&self.view()).read().unwrap()
    }

    pub fn caller_read_u16(&self, ptr: u32) -> u16 {
        let ptr: WasmPtr<u16> = WasmPtr::new(ptr);
        ptr.deref(&self.view()).read().unwrap()
    }

    pub fn caller_read_u32(&self, ptr: u32) -> u32 {
        let ptr: WasmPtr<u32> = WasmPtr::new(ptr);
        ptr.deref(&self.view()).read().unwrap()
    }

    pub fn caller_read_u64(&self, ptr: u32) -> u64 {
        let ptr: WasmPtr<u64> = WasmPtr::new(ptr);
        ptr.deref(&self.view()).read().unwrap()
    }

    pub fn caller_write_u8(&mut self, ptr: u32, x: u8) -> &mut Self {
        let ptr: WasmPtr<u8> = WasmPtr::new(ptr);
        ptr.deref(&self.view()).write(x).unwrap();
        self
    }

    pub fn caller_write_u16(&mut self, ptr: u32, x: u16) -> &mut Self {
        let ptr: WasmPtr<u16> = WasmPtr::new(ptr);
        ptr.deref(&self.view()).write(x).unwrap();
        self
    }

    pub fn caller_write_u32(&mut self, ptr: u32, x: u32) -> &mut Self {
        let ptr: WasmPtr<u32> = WasmPtr::new(ptr);
        ptr.deref(&self.view()).write(x).unwrap();
        self
    }

    pub fn caller_write_u64(&mut self, ptr: u32, x: u64) -> &mut Self {
        let ptr: WasmPtr<u64> = WasmPtr::new(ptr);
        ptr.deref(&self.view()).write(x).unwrap();
        self
    }
