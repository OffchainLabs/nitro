...
use crate::transfer::{receive_response, receive_validation_input, send_failure_response, send_successful_response, send_validation_input};
...
fn transfer_input() -> Result<(), Box<dyn std::error::Error>> {
    let input = ValidationInput {
        ...
        batch_info: vec![
            BatchInfo { number: 10, data: vec![1, 2, 3] },
            BatchInfo { number: 11, data: vec![4, 5, 6] },
            BatchInfo { number: 12, data: vec![7, 8] },
        ],
        ...
        preimages: HashMap::from([
            (PreimageType::Keccak256, HashMap::from([
                (Bytes32::from([0u8; 32]), vec![0xDE, 0xAD, 0xBE, 0xEF]),
                (Bytes32::from([1u8; 32]), vec![0xBA, 0xAD, 0xF0, 0x0D]),
            ])),
            (PreimageType::DACertificate, HashMap::from([
                (Bytes32::from([2u8; 32]), vec![0xFE, 0xED, 0xFA, 0xCE]),
            ])),
        ]),
        user_wasms: HashMap::from([
            (local_target(), HashMap::from([
