...
use std::{ffi::{CStr, CString}, fs::File, io::Read, os::raw::c_char, path::Path};
...
#[no_mangle]
pub unsafe extern "C" fn arbitrator_load_machine(binary_path: *const c_char, library_paths: *const *const c_char, library_paths_size: isize, always_merkelize: bool) -> *mut Machine {
	match arbitrator_load_machine_impl(binary_path, library_paths, library_paths_size, always_merkelize) {
		Ok(mach) => {mach}
		Err(err) => {
...
unsafe fn arbitrator_load_machine_impl(binary_path: *const c_char, library_paths: *const *const c_char, library_paths_size: isize, always_merkelize: bool) -> Result<*mut Machine> {
	let main_mod = {
...
pub unsafe extern "C" fn arbitrator_gen_proof(mach: *mut Machine) -> *mut c_char {
	...
	CString::new(hex::encode(proof)).expect("CString new failed").into_raw()
}
