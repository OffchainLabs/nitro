...
func (con ArbRetryableTx) Redeem(c ctx, evm mech, ticketId bytes32) (bytes32, error) {
	...
	// We burn as much special fee gas as we can, leaving only enough to pay for copying out the return data.
	const donationResource = multigas.ResourceKindSpecialFee
	...
	// Even though the special fee backlog doesn't affect multi-gas prices, we still do this in case multi-gas is disabled.
