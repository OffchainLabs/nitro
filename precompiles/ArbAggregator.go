...
func (con ArbAggregator) GetFeeCollector(c ctx, evm mech, aggregator addr) (addr, error) {
	...
	return arbosState.OpenArbosState(evm.StateDB).L1PricingState().AggregatorFeeCollector(aggregator), nil
}
...
func (con ArbAggregator) GetPreferredAggregator(c ctx, evm mech, address addr) (addr, bool, error) {
	...
	res, exists := arbosState.OpenArbosState(evm.StateDB).L1PricingState().PreferredAggregator(address)
...
func (con ArbAggregator) GetTxBaseFee(c ctx, evm mech, aggregator addr) (huge, error) {
	...
	return arbosState.OpenArbosState(evm.StateDB).L1PricingState().FixedChargeForAggregatorL1Gas(aggregator), nil
}
...
func (con ArbAggregator) SetFeeCollector(c ctx, evm mech, aggregator addr, newFeeCollector addr) error {
	...
	l1State := arbosState.OpenArbosState(evm.StateDB).L1PricingState()
...
func (con ArbAggregator) SetPreferredAggregator(c ctx, evm mech, prefAgg addr) error {
	...
	arbosState.OpenArbosState(evm.StateDB).L1PricingState().SetPreferredAggregator(c.caller, prefAgg)
	return nil
}
...
func (con ArbAggregator) SetTxBaseFee(c ctx, evm mech, aggregator addr, feeInL1Gas huge) error {
	...
	arbosState.OpenArbosState(evm.StateDB).L1PricingState().SetFixedChargeForAggregatorL1Gas(aggregator, feeInL1Gas)
	return nil
}
