...
const TransactionCensorshipEnableDelay = 7 * 24 * 60 * 60
...
var (
	...
	ErrTransactionCensorshipDelay    = errors.New("transaction filtering feature must be enabled at least 7 days in the future")
	ErrTransactionCensorshipBackward = errors.New("transaction filtering feature cannot be updated to a time earlier than the current time at which it is scheduled to be enabled")
)
...
// SetTransactionCensorshipFrom sets a time in epoch seconds when the transaction censorship
// feature becomes enabled. Setting it to 0 disables the feature.
// If the feature is disabled, then the time must be at least 7 days in the
// future.
func (con ArbOwner) SetTransactionCensorshipFrom(c ctx, evm mech, timestamp uint64) error {
	if timestamp == 0 {
		return c.State.SetTransactionCensorshipFromTime(0)
	}
	stored, err := c.State.TransactionCensorshipFromTime()
	if err != nil {
		return err
	}
	now := evm.Context.Time
	...
	if err := validateFeatureFromTimeUpdate(
		...
		now,
		timestamp,
		TransactionCensorshipEnableDelay,
		ErrTransactionCensorshipDelay,
		ErrTransactionCensorshipBackward,
	); err != nil {
		return err
	}

	return c.State.SetTransactionCensorshipFromTime(timestamp)
}
