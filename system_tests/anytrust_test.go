...
func TestAnyTrustRekey(t *testing.T) {
	...
	testStart := time.Now()
	phaseStart := testStart

	// Setup L1 chain and contracts
	builder := NewNodeBuilder(ctx).DefaultConfig(t, true).WithExtraWeight(2)
	builder.BuildL1(t)
	t.Logf("phase BuildL1: %s (total %s)", time.Since(phaseStart), time.Since(testStart))
	phaseStart = time.Now()
	...
	t.Logf("phase startLocalAnyTrustServerA: %s (total %s)", time.Since(phaseStart), time.Since(testStart))
	phaseStart = time.Now()
	...
	l1NodeConfigB := arbnode.ConfigDefaultL1NonSequencerTest()
	{
		...
		t.Logf("phase authorizeKeysetA: %s (total %s)", time.Since(phaseStart), time.Since(testStart))
		phaseStart = time.Now()
		...
		t.Logf("phase BuildL2OnL1 (1st): %s (total %s)", time.Since(phaseStart), time.Since(testStart))
		phaseStart = time.Now()
		...
		t.Logf("phase Build2ndNode (1st): %s (total %s)", time.Since(phaseStart), time.Since(testStart))
		phaseStart = time.Now()

		checkBatchPosting(t, ctx, builder, l2B.Client)
		t.Logf("phase checkBatchPosting (1st): %s (total %s)", time.Since(phaseStart), time.Since(testStart))
		phaseStart = time.Now()

		builder.L2.cleanup()
		t.Logf("phase L2.cleanup (1st): %s (total %s)", time.Since(phaseStart), time.Since(testStart))
		phaseStart = time.Now()

		cleanupB()
		t.Logf("phase cleanupB (1st): %s (total %s)", time.Since(phaseStart), time.Since(testStart))
		phaseStart = time.Now()
	}

	err := anyTrustRpcServerA.Shutdown(ctx)
	Require(t, err)
	t.Logf("phase shutdownServerA: %s (total %s)", time.Since(phaseStart), time.Since(testStart))
	phaseStart = time.Now()
	...
	t.Logf("phase startLocalAnyTrustServerB: %s (total %s)", time.Since(phaseStart), time.Since(testStart))
	phaseStart = time.Now()

	authorizeAnyTrustKeyset(t, ctx, pubkeyB, builder.L1Info, builder.L1.Client)
	t.Logf("phase authorizeKeysetB: %s (total %s)", time.Since(phaseStart), time.Since(testStart))
	phaseStart = time.Now()
	...
	defer cleanup()
	t.Logf("phase BuildL2OnL1 (2nd): %s (total %s)", time.Since(phaseStart), time.Since(testStart))
	phaseStart = time.Now()
	...
	defer cleanup()
	t.Logf("phase Build2ndNode (2nd): %s (total %s)", time.Since(phaseStart), time.Since(testStart))
	phaseStart = time.Now()

	checkBatchPosting(t, ctx, builder, l2B.Client)
	t.Logf("phase checkBatchPosting (2nd): %s (total %s)", time.Since(phaseStart), time.Since(testStart))
}
