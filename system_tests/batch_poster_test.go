...
func TestBatchPosterParallel(t *testing.T) {
	...
	for i := 0; i < parallelBatchPosters; i++ {
		batchPoster, err := arbnode.NewBatchPoster(nodeA.L1Reader, nodeA.InboxTracker, nodeA.TxStreamer, nodeA.SyncMonitor, &conf.BatchPoster, nodeA.DeployInfo.SequencerInbox, &seqTxOpts, nil)
		Require(t, err)
