...
// TestCustomDAValidation tests validation functionality with CustomDA
func TestCustomDAValidation(t *testing.T) {
	if testing.Short() {
	...
	// Apply the same CustomDA configuration to the validator
	configureNodeForCustomDA(validatorConfig, serverURL)

	// Make sure validator is set up for validation
	validatorConfig.Staker.Enable = true

	// Build validator node
	validatorTestClient, validatorCleanup := builder.Build2ndNode(t, &SecondNodeParams{
	...
	// Verify the validator node received and validated the transactions
	balance, err := validatorTestClient.Client.BalanceAt(ctx, builder.L2Info.GetAddress("User2"), nil)
	Require(t, err)
	expectedBalance := new(big.Int).Mul(perTransfer, big.NewInt(3))
	if balance.Cmp(expectedBalance) != 0 {
		Fatal(t, "Unexpected balance on validator node:", balance, "expected:", expectedBalance)
	}

	log.Info("CustomDA validation test completed successfully")
}
