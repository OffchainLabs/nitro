...
func TestRemoteValidator(t *testing.T) {
	...
	builder.l2StackConfig.HTTPHost = node.DefaultHTTPHost
	builder.l2StackConfig.HTTPPort = node.DefaultHTTPPort
	builder.l2StackConfig.HTTPModules = append(builder.l2StackConfig.HTTPModules, "arb")
	builder.L2Info = NewBlockChainTestInfo(
		t,
		types.NewArbitrumSigner(types.NewLondonSigner(builder.chainConfig.ChainID)), big.NewInt(l2pricing.InitialBaseFeeWei*2),
		transferGas,
	...
	config.BatchPoster.Enable = false
	builder.execConfig.Sequencer.Enable = false
	builder.l2StackConfig.HTTPHost = ""
	...
	err = statelessA.Start(ctx)
	Require(t, err)
	blockValidatorConfig.RemoteBlockValidatorServer = rpcclient.TestClientConfig
	blockValidatorConfig.RemoteBlockValidatorServer.URL = "http://localhost:8545"
	blockValidatorA, err := staker.NewBlockValidator(
		ctx, statelessA, l2nodeA.InboxTracker, l2nodeA.TxStreamer, func() *staker.BlockValidatorConfig { return &blockValidatorConfig }, make(chan error, 10), valStack,
	)
	Require(t, err)
	err = blockValidatorA.Start(ctx)
	Require(t, err)
	stakerA, err := staker.NewStaker(
		l2nodeA.L1Reader,
		valWalletA,
