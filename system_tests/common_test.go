...
func CreateTestL2WithConfig(
	...
	currentNode, err := arbnode.New(ctx, stack, chainDb, arbDb, NewFetcherFromConfig(nodeConfig), blockchain, nil, nil, nil, nil, nil, feedErrChan)
	Require(t, err)
...
func Create2ndNodeWithConfig(
	...
	currentNode, err := arbnode.New(ctx, l2stack, l2chainDb, l2arbDb, NewFetcherFromConfig(nodeConfig), l2blockchain, l1client, first.DeployInfo, &txOpts, &txOpts, dataSigner, feedErrChan)
	Require(t, err)
