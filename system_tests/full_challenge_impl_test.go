...
func RunChallengeTest(t *testing.T, asserterIsCorrect bool, useStubs bool, challengeMsgIdx int64) {
	...
	asserterValidator, err := staker.NewStatelessBlockValidator(asserterL2.InboxReader, asserterL2.InboxTracker, asserterL2.TxStreamer, asserterExec.Recorder, asserterL2ArbDb, nil, StaticFetcherFrom[*staker.BlockValidatorConfig](&conf.BlockValidator), valStack)
	if err != nil {
		Fail(t, err)
	}
	if useStubs {
	...
	challengerValidator, err := staker.NewStatelessBlockValidator(challengerL2.InboxReader, challengerL2.InboxTracker, challengerL2.TxStreamer, challengerExec.Recorder, challengerL2ArbDb, nil, StaticFetcherFrom[*staker.BlockValidatorConfig](&conf.BlockValidator), valStack)
	if err != nil {
		Fail(t, err)
	}
	if useStubs {
