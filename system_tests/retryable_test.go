...
func TestRetryableSubmissionAndAutoRedeemFees(t *testing.T) {
	...
	burnTarget := uint64(1 * l2pricing.InitialSpeedLimitPerSecondV6 * l2pricing.InitialBacklogTolerance)
...
func TestRetryableSubmissionAndRedeemFees(t *testing.T) {
	...
	burnAmount := arbnode.ConfigDefaultL1Test().RPC.RPCGasCap / 2
	burnTarget := uint64(3 * l2pricing.InitialSpeedLimitPerSecondV6 * l2pricing.InitialBacklogTolerance)
	for i := uint64(0); i < (burnTarget+burnAmount)/burnAmount; i++ {
		burnArbGas := arbostestabi.Methods["burnArbGas"]
		data, err := burnArbGas.Inputs.Pack(arbmath.UintToBig(burnAmount - l2info.TransferGas))
		Require(t, err)
