...
export function namedAccount(name: string, threadId?: number | undefined): ethers.Wallet {
    if (name == "funnel") {
    ...
    if (name.startsWith("threaduser_")) {
        ...
        return new ethers.Wallet(ethers.utils.sha256(ethers.utils.toUtf8Bytes(name.substring(6) + "_thread_" + threadId.toString())))
    }
...
export function namedAddress(name: string, threadId?: number | undefined): string {
...
export const printAddressCommand = {
    ...
    builder: {
        account: { string: true, describe: "address (see general help)", default: "funnel" },
    },
