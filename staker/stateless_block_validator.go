...
func NewStatelessBlockValidator(
	...
	for i := range configs {
		...
		if i == 0 {
			executionSpawners = append(executionSpawners, validatorclient.NewBoldExecutionClient(confFetcher, redisValClient, &config().RedisValidationClientConfig, stack, executionSpawner))
		}
	}
