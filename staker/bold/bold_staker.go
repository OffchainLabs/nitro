...
func NewBOLDStaker(
	...
	manager, err := newBOLDChallengeManager(ctx, stack, rollupAddress, txOpts, l1Reader, wrappedClient, blockValidator, statelessBlockValidator, config, dataPoster, inboxReader, inboxTracker, streamer)
	if err != nil {
		return nil, err
	}
	return &BOLDStaker{
...
func (b *BOLDStaker) getLatestState(ctx context.Context, confirmed bool) (arbutil.MessageIndex, *validator.GoGlobalState, error) {
	...
	caughtUp, count, err := staker.GlobalStateToMsgCount(b.inboxTracker, b.streamer, validator.GoGlobalState(globalState))
	if err != nil {
	...
	processedCount, err := b.streamer.GetProcessedMessageCount()
	if err != nil {
		return 0, nil, err
	}
