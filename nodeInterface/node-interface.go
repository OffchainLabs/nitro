...
func arbNodeFromNodeInterfaceBackend(backend core.NodeInterfaceBackendAPI) (*arbnode.Node, error) {
...
func getL1Confirmations(node *arbnode.Node, blockHash common.Hash) (uint64, error) {
...
func init() {
	...
	/*nodeInterfaceOld, err := abi.JSON(strings.NewReader(node_interfacegen.NodeInterfaceABI))
	if err != nil {
		panic(err)
	}
	nodeInterfaceDebugOld, err := abi.JSON(strings.NewReader(node_interfacegen.NodeInterfaceDebugABI))
	if err != nil {
		panic(err)
	}*/
	...
	resetInterfaceImpls := func(msg Message, backend core.NodeInterfaceBackendAPI) {
		nodeInterfaceImpl.backend = backend
		nodeInterfaceImpl.sourceMessage = msg
		nodeInterfaceImpl.returnMessage = nil
		nodeInterfaceDebugImpl.backend = backend
		nodeInterfaceDebugImpl.sourceMessage = msg
		nodeInterfaceDebugImpl.returnMessage = nil
	}
	readInterfaceImpls := func() *Message {
		if nodeInterfaceImpl.returnMessage != nil {
			return nodeInterfaceImpl.returnMessage
		}
		if nodeInterfaceDebugImpl.returnMessage != nil {
			return nodeInterfaceDebugImpl.returnMessage
		}
		return nil
	}
	...
	core.InterceptRPCMessage = func(
		...
		if to != nil && arbosVersion != 0 {
			...
			resetInterfaceImpls(msg, backend)
			...
			if alternativeMsg := readInterfaceImpls(); alternativeMsg != nil {
				return *alternativeMsg, nil, nil
			}
			res := &ExecutionResult{
