...
func (m *MessageExtractor) reorg(ctx context.Context, current *fsm.CurrentState[action, FSMState]) (time.Duration, error) {
	...
	// Notify consumers immediately after the rewrite is persisted, before
	// transitioning to processNextBlock. This prevents lost notifications
	// if the node crashes between the DB write and the next FSM step.
	if m.blockValidator != nil {
		m.blockValidator.ReorgToBatchCount(previousState.BatchCount)
	}
	if err := m.sendReorgNotification(ctx, previousState.ParentChainBlockNumber); err != nil {
		return 0, err
	}
