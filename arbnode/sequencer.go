...
func NewSequencer(txStreamer *TransactionStreamer, l1Reader *headerreader.HeaderReader, configFetcher SequencerConfigFetcher) (*Sequencer, error) {
	...
	entries := strings.Split(configFetcher().SenderWhitelist, ",")
...
func (s *Sequencer) postTxFilter(state *arbosState.ArbosState, tx *types.Transaction, sender common.Address, dataGas uint64, result *core.ExecutionResult) error {
	if result.Err != nil && result.UsedGas > dataGas && result.UsedGas-dataGas <= s.configFetcher().MaxRevertGasReject {
...
func (s *Sequencer) ForwardTo(url string) error {
	...
	s.forwarder = NewForwarder(url, s.configFetcher().ForwardTimeout)
...
func (s *Sequencer) sequenceTransactions(ctx context.Context) {
	...
	if s.l1Reader != nil && (l1Block == 0 || math.Abs(float64(l1Timestamp)-float64(timestamp)) > s.configFetcher().MaxAcceptableTimestampDelta.Seconds()) {
		log.Error(
...
func (s *Sequencer) Start(ctxIn context.Context) error {
	...
	s.CallIteratively(func(ctx context.Context) time.Duration {
		nextBlock := time.Now().Add(s.configFetcher().MaxBlockSpeed)
