...
func (p *DataPoster) postTransactionWithMutex(ctx context.Context, dataCreatedAt time.Time, nonce uint64, meta []byte, to common.Address, calldata []byte, gasLimit uint64, value *big.Int, kzgBlobs []kzg4844.Blob, accessList types.AccessList) (*types.Transaction, error) {
	...
	if len(kzgBlobs) > 0 {
		...
		proofs, version, err := blobs.ComputeBlobProofs(kzgBlobs, commitments, p.config().EnableCellProofs)
		if err != nil {
...
type DataPosterConfig struct {
	...
	EnableCellProofs       bool              `koanf:"enable-cell-proofs" reload:"hot"`
...
func DataPosterConfigAddOptions(prefix string, f *pflag.FlagSet, defaultDataPosterConfig DataPosterConfig) {
	...
	f.Bool(prefix+".enable-cell-proofs", defaultDataPosterConfig.EnableCellProofs, "enable cell proofs in blob transactions for Fusaka compatibility")
...
var DefaultDataPosterConfig = DataPosterConfig{
	...
	NonceRbfSoftConfs:      1,
	Post4844Blobs:          false,
	EnableCellProofs:       false, // TODO: Change default after Fusaka activates on L1
