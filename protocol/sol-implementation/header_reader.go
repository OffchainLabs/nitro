...
func NewHeaderReader(client util.L1Interface, config ConfigFetcher) *HeaderReader {
	return &HeaderReader{
		client:            client,
		config:            config,
...
func (s *HeaderReader) possiblyBroadcast(h *types.Header) {
	...
	if s.requiresPendingCallUpdates > 0 {
		pendingCallBlockNr, err := util.GetPendingCallBlockNumber(s.GetContext(), s.client)
	...
	for {
		...
		if err == nil && receipt.BlockNumber.IsUint64() {
			...
			if callBlockNr > receiptBlockNr {
				return receipt, util.DetailTxError(ctx, s.client, tx, receipt)
			}
		}
		select {
...
func (s *HeaderReader) Client() util.L1Interface {
	return s.client
}
