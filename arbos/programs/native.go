...
func getCompiledProgram(statedb vm.StateDB, moduleHash common.Hash, addressForLogging common.Address, code []byte, codehash common.Hash, maxWasmSize uint32, pagelimit uint16, time uint64, debugMode bool, program Program, runCtx *core.MessageRunContext) (map[rawdb.WasmTarget][]byte, error) {
	...
	shouldAllowFallback := allowFallback.Load() && runCtx.IsExecutedOnChain()
	info, newlyBuilt, err := activateProgramInternal(addressForLogging, codehash, wasm, pagelimit, program.version, zeroArbosVersion, debugMode, &zeroGas, missingTargets, moduleActivationMandatory, shouldAllowFallback)
	if err != nil {
...
func callProgram(
	...
	if status == userNativeStackOverflow {
		// A native (host) stack overflow during stylus execution is a consensus-breaking
		// failure: a node that cannot finish a call would produce different state than
		// nodes that can. Panic rather than risk divergence.
