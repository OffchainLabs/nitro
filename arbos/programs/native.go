...
func cacheProgram(db vm.StateDB, module common.Hash, program Program, address common.Address, code []byte, codeHash common.Hash, params *StylusParams, debug bool, time uint64, runMode core.MessageRunMode) {
	if runMode == core.MessageCommitMode {
		// address is only used for logging
		asm, err := getLocalAsm(db, module, address, code, codeHash, params.PageLimit, time, debug, program)
		if err != nil {
