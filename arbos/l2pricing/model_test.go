...
func TestCalcMultiGasConstraintsExponents(t *testing.T) {
	...
	if got := exponents[multigas.ResourceKindSpecialFee]; got != 0 {
		t.Errorf("expected zero special fee exponent, got %v", got)
	}
...
func TestMultiDimensionalPriceForRefund(t *testing.T) {
	...
	for k := 0; k < int(multigas.NumResourceKind); k++ {
		...
		if kind == multigas.ResourceKindSpecialFee || baseFeeKind.Sign() == 0 {
