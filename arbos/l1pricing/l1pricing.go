...
type L1PricingState struct {
	...
	l1GasPriceEstimate          *storage.StorageBackedUbig
...
func InitializeL1PricingState(sto *storage.Storage) {
	...
	sto.OpenStorageBackedUbig(util.UintToHash(l1GasPriceEstimateOffset)).Set(big.NewInt(50 * params.GWei))
}
...
func OpenL1PricingState(sto *storage.Storage) *L1PricingState {
	...
	return &L1PricingState{
		sto,
		defaultAggregator,
		sto.OpenStorageBackedUbig(util.UintToHash(l1GasPriceEstimateOffset)),
...
func (ps *L1PricingState) UpdateL1GasPriceEstimate(baseFeeWei *big.Int) {
	ps.l1GasPriceEstimate.Set(new(big.Int).Div(
		new(big.Int).Add(
			baseFeeWei,
			new(big.Int).Mul(ps.l1GasPriceEstimate.Get(), big.NewInt(L1GasPriceEstimateSamplesInAverage-1)),
		),
