...
func (p *TxProcessor) EndTxHook(gasLeft uint64, success bool) {
	...
	if underlyingTx != nil && underlyingTx.Type() == types.ArbitrumRetryTxType {
		...
		p.state.Restrict(p.state.L2PricingState().AddToGasPoolWithArbosVersion(-arbmath.SaturatingCast[int64](gasUsed), p.state.ArbOSVersion()))
		return
	}
	...
	if p.msg.GasPrice.Sign() > 0 { // in tests, gas price could be 0
		...
		p.state.Restrict(p.state.L2PricingState().AddToGasPoolWithArbosVersion(-arbmath.SaturatingCast[int64](computeGas), p.state.ArbOSVersion()))
	}
}
