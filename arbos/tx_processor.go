...
func (p *TxProcessor) EndTxHook(gasLeft uint64, usedMultiGas multigas.MultiGas, success bool) {
	...
	if underlyingTx != nil && underlyingTx.Type() == types.ArbitrumRetryTxType {
		...
		p.state.Restrict(p.state.L2PricingState().AddToGasPool(true, gasUsed, usedMultiGas, p.state.ArbOSVersion()))
		return
	}
	...
	if p.msg.GasPrice.Sign() > 0 { // in tests, gas price could be 0
		...
		p.state.Restrict(p.state.L2PricingState().AddToGasPool(true, computeGas, usedMultiGas, p.state.ArbOSVersion()))
	}
}
