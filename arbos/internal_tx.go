...
func ApplyInternalTxUpdate(tx *types.ArbitrumInternalTx, state *arbosState.ArbosState, evm *vm.EVM) error {
	...
	case InternalTxStartBlockMethodID:
		...
		if l1BlockNumber > oldL1BlockNumber {
			state.Restrict(state.Blockhashes().RecordNewL1Block(evm.Config.Tracer, l1BlockNumber-1, prevHash, state.ArbOSVersion()))
		}
		...
		state.L2PricingState().UpdatePricingModel(timePassed, evm.Config.Tracer)
