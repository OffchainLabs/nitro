...
wasms=$(patsubst %.wat,%.wasm, $(inputs)) $(patsubst prover/test-cases/rust/src/bin/%.rs,prover/test-cases/rust/target/wasm32-unknown-unknown/debug/%.wasm, $(rust_bin_sources))
...
prover/test-cases/rust/target/wasm32-unknown-unknown/debug/%.wasm: prover/test-cases/rust/src/bin/%.rs prover/test-cases/rust/src/lib.rs
	cd prover/test-cases/rust && cargo +nightly build --target wasm32-unknown-unknown --bin $(patsubst prover/test-cases/rust/target/wasm32-unknown-unknown/debug/%.wasm,%, $@)
