...
.make/lint: $(DEP_PREDICATE) build-node-deps $(ORDER_ONLY_PREDICATE) .make
	...
	# Yields several non-optimally sorted structs, not sure this is very 
	# for us. Probably fine to not have this linter.
	# go run ./linter/maligned ./...

	# Don't think this is really relevant for us.
	# go run ./linter/properpermissions ./...
