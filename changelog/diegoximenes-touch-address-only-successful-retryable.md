...
- `TxFailed` deduplicates `filteredTxHashes`: a filtered sender touched by both the submission tx and its auto-redeem no longer records the same originating hash twice in the halt state. Both the submission's and the cascading redeem's filter reports are still emitted.
