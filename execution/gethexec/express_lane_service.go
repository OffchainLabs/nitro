...
func (es *expressLaneService) sequenceExpressLaneSubmission(
	...
	if msg.SequenceNumber > roundInfo.sequence {
		...
		if msg.SequenceNumber > roundInfo.sequence+seqConfig.Timeboost.MaxFutureSequenceDistance {
			return fmt.Errorf("message sequence number has reached max allowed limit. SequenceNumber: %d, Limit: %d", msg.SequenceNumber, roundInfo.sequence+seqConfig.Timeboost.MaxFutureSequenceDistance)
		}
