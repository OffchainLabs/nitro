...
func StylusTargetConfigAddOptions(prefix string, f *pflag.FlagSet) {
	...
	f.Uint64(prefix+".native-stack-size", DefaultStylusTargetConfig.NativeStackSize, "initial native stack size in bytes for Wasmer coroutines used by Stylus execution (0 = default 1MB). On stack overflow the size is automatically doubled up to 100MB")
}
