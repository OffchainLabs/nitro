...
func (s *ExecutionEngine) ResequenceReorgedMessages(messages []*arbostypes.MessageWithMetadata) {
	...
	for _, msg := range messages {
		...
		_, _, err = s.sequenceTransactionsWithBlockMutex(msg.Message.Header, hooks, nil)
		if err != nil {
...
func (s *ExecutionEngine) SequenceTransactions(header *arbostypes.L1IncomingMessageHeader, hooks *arbos.SequencingHooks, timeboostedTxs map[common.Hash]struct{}) (*execution.SequencedMsg, *types.Block, error) {
...
func (s *ExecutionEngine) SequenceTransactionsWithProfiling(header *arbostypes.L1IncomingMessageHeader, hooks *arbos.SequencingHooks, timeboostedTxs map[common.Hash]struct{}) (*execution.SequencedMsg, *types.Block, error) {
...
func (s *ExecutionEngine) AppendLastSequencedBlock(blockHash common.Hash) error {
	...
	if s.multigasCollector != nil {
		s.multigasCollector.FinaliseBlock(multigascollector.BlockInfo{
			BlockNumber:    s.lastSequencedBlockInfo.block.NumberU64(),
			BlockHash:      s.lastSequencedBlockInfo.block.Hash().Bytes(),
			BlockTimestamp: s.lastSequencedBlockInfo.block.Time(),
		})
	}
...
func (s *ExecutionEngine) sequenceTransactionsWithBlockMutex(header *arbostypes.L1IncomingMessageHeader, hooks *arbos.SequencingHooks, timeboostedTxs map[common.Hash]struct{}) (*execution.SequencedMsg, *types.Block, error) {
	lastBlockHeader, err := s.getCurrentHeader()
	if err != nil {
		return nil, nil, err
	}
