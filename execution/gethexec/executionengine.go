...
func touchAddresses(db *state.StateDB, tx *types.Transaction, sender common.Address) {
	filterSetID := db.FilterSetID()
	db.TouchAddress(&filter.FilteredAddressRecord{FilterSetID: filterSetID, Address: sender, FilterReason: filter.FilterReason{Reason: filter.ReasonFrom, EventRuleMatch: nil}})
	if tx.To() != nil {
		db.TouchAddress(&filter.FilteredAddressRecord{FilterSetID: filterSetID, Address: *tx.To(), FilterReason: filter.FilterReason{Reason: filter.ReasonTo, EventRuleMatch: nil}})
	}
	...
	if arbosutil.DoesTxTypeAlias(&txType) {
		db.TouchAddress(&filter.FilteredAddressRecord{FilterSetID: filterSetID, Address: arbosutil.InverseRemapL1Address(sender), FilterReason: filter.FilterReason{Reason: filter.ReasonDealiasedFrom, EventRuleMatch: nil}})
	}
...
func applyEventFilter(ef *eventfilter.EventFilter, db *state.StateDB) {
	...
	for _, l := range logs {
		for _, record := range ef.AddressesForFiltering(l.Topics, l.Data, l.Address, filterSetID) {
...
func touchRetryableAddresses(db *state.StateDB, tx *types.Transaction) {
	if inner, ok := tx.GetInner().(*types.ArbitrumSubmitRetryableTx); ok {
		filterSetID := db.FilterSetID()
		db.TouchAddress(&filter.FilteredAddressRecord{FilterSetID: filterSetID, Address: inner.Beneficiary, FilterReason: filter.FilterReason{Reason: filter.ReasonRetryableBeneficiary, EventRuleMatch: nil}})
		db.TouchAddress(&filter.FilteredAddressRecord{FilterSetID: filterSetID, Address: inner.FeeRefundAddr, FilterReason: filter.FilterReason{Reason: filter.ReasonRetryableFeeRefund, EventRuleMatch: nil}})
		if inner.RetryTo != nil {
			db.TouchAddress(&filter.FilteredAddressRecord{FilterSetID: filterSetID, Address: *inner.RetryTo, FilterReason: filter.FilterReason{Reason: filter.ReasonRetryableTo, EventRuleMatch: nil}})
		}
		db.TouchAddress(&filter.FilteredAddressRecord{FilterSetID: filterSetID, Address: arbosutil.InverseRemapL1Address(inner.Beneficiary), FilterReason: filter.FilterReason{Reason: filter.ReasonDealiasedRetryableBeneficiary, EventRuleMatch: nil}})
		db.TouchAddress(&filter.FilteredAddressRecord{FilterSetID: filterSetID, Address: arbosutil.InverseRemapL1Address(inner.FeeRefundAddr), FilterReason: filter.FilterReason{Reason: filter.ReasonDealiasedRetryableFeeRefund, EventRuleMatch: nil}})
	}
}
