...
func TestVerifyPrefixProof_GoSolidityEquivalence(t *testing.T) {
	...
	startMessageNumber := layer2stateprovider.Height(0)
	fromMessageNumber := layer2stateprovider.Height(3)
	toMessageNumber := layer2stateprovider.Height(7)
	req := &layer2stateprovider.HistoryCommitmentRequest{
		...
		UpToHeight:                  option.Some(layer2stateprovider.Height(fromMessageNumber)),
	}
	loCommit, err := manager.HistoryCommitment(ctx, req)
	require.NoError(t, err)

	req.UpToHeight = option.Some(layer2stateprovider.Height(toMessageNumber))
...
func TestVerifyPrefixProofWithHeight7_GoSolidityEquivalence1(t *testing.T) {
	...
	startMessageNumber := layer2stateprovider.Height(0)
	fromMessageNumber := layer2stateprovider.Height(3)
	toMessageNumber := layer2stateprovider.Height(6)
	req := &layer2stateprovider.HistoryCommitmentRequest{
		AssertionMetadata: &layer2stateprovider.AssociatedAssertionMetadata{
			WasmModuleRoot: wasmModuleRoot,
			FromState: chainabstraction.GoGlobalState{
...
func FuzzPrefixProof_Verify(f *testing.F) {
	...
	wasmModuleRoot := common.Hash{}
	batch := layer2stateprovider.Batch(1)
	req := &layer2stateprovider.HistoryCommitmentRequest{
		AssertionMetadata: &layer2stateprovider.AssociatedAssertionMetadata{
			WasmModuleRoot: wasmModuleRoot,
			FromState: chainabstraction.GoGlobalState{
