...
type Assertion struct {
	...
	id        chainabstraction.AssertionHash
	createdAt uint64
	...
	prevId           option.Option[chainabstraction.AssertionHash] // This is set the first time prevId is called
...
func (a *Assertion) Id() chainabstraction.AssertionHash {
	return a.id
}

func (a *Assertion) PrevId(ctx context.Context) (chainabstraction.AssertionHash, error) {
	if a.prevId.IsSome() {
...
func (a *Assertion) inner(ctx context.Context, opts *bind.CallOpts) (*rollupgen.AssertionNode, error) {
	...
	assertionStatus := chainabstraction.AssertionStatus(assertionNode.Status)
	if assertionStatus == chainabstraction.AssertionConfirmed {
		a.isConfirmed = true
	}
...
func (a *Assertion) Status(ctx context.Context, opts *bind.CallOpts) (chainabstraction.AssertionStatus, error) {
	if a.isConfirmed {
		return chainabstraction.AssertionConfirmed, nil
	}
	inner, err := a.inner(ctx, opts)
	if err != nil {
		return 0, err
	}
	return chainabstraction.AssertionStatus(inner.Status), nil
}
...
type specEdge struct {
	...
	lowerChild        option.Option[chainabstraction.EdgeId] // Once the edge has a lower child, this is set
	upperChild        option.Option[chainabstraction.EdgeId] // Once the edge has an upper child, this is set
