...
package solimplementation_test
...
func TestEdgeChallengeManager_IsUnrivaled(t *testing.T) {
	...
	leafAdder := func(stateManager layer2stateprovider.Provider, leaf chainabstraction.Assertion) chainabstraction.VerifiedRoyalEdge {
		req := &layer2stateprovider.HistoryCommitmentRequest{
			...
			UpToHeight:                  option.Some(layer2stateprovider.Height(0)),
		}
		startCommit, startErr := stateManager.HistoryCommitment(ctx, req)
		require.NoError(t, startErr)

		req.UpToHeight = option.Some(layer2stateprovider.Height(challengetesting.LevelZeroBlockEdgeHeight))
	...
	require.Equal(t, chainabstraction.NewBlockChallengeLevel(), challengeLevel)
	...
	t.Run("bisected children are presumptive", func(t *testing.T) {
		var bisectHeight uint64 = challengetesting.LevelZeroBlockEdgeHeight / 2
		req := &layer2stateprovider.HistoryCommitmentRequest{
			AssertionMetadata:           simpleAssertionMetadata(),
			UpperChallengeOriginHeights: []layer2stateprovider.Height{},
			UpToHeight:                  option.Some(layer2stateprovider.Height(bisectHeight)),
		}
		...
		req = &layer2stateprovider.HistoryCommitmentRequest{
			AssertionMetadata:           simpleAssertionMetadata(),
			UpperChallengeOriginHeights: []layer2stateprovider.Height{},
			UpToHeight:                  option.Some(layer2stateprovider.Height(challengetesting.LevelZeroBlockEdgeHeight)),
		}
		honestProof, err := createdData.HonestStateManager.PrefixProof(ctx, req, layer2stateprovider.Height(bisectHeight))
		require.NoError(t, err)
...
func TestEdgeChallengeManager_HasLengthOneRival(t *testing.T) {
	...
	t.Run("post bisection, mutual edge is one step fork source", func(t *testing.T) {
		var height uint64 = challengetesting.LevelZeroBlockEdgeHeight
		for height > 1 {
			req := &layer2stateprovider.HistoryCommitmentRequest{
				AssertionMetadata:           simpleAssertionMetadata(),
				UpperChallengeOriginHeights: []layer2stateprovider.Height{},
				UpToHeight:                  option.Some(layer2stateprovider.Height(height / 2)),
			}
			honestBisectCommit, err := honestStateManager.HistoryCommitment(ctx, req)
			require.NoError(t, err)
			prefixCommitReq := &layer2stateprovider.HistoryCommitmentRequest{
				AssertionMetadata:           simpleAssertionMetadata(),
				UpperChallengeOriginHeights: []layer2stateprovider.Height{},
				UpToHeight:                  option.Some(layer2stateprovider.Height(height)),
			}
			honestProof, err := honestStateManager.PrefixProof(
				ctx,
				prefixCommitReq,
				layer2stateprovider.Height(height/2),
			)
			require.NoError(t, err)
			...
			require.NoError(t, err)
			evilProof, err := evilStateManager.PrefixProof(ctx, prefixCommitReq, layer2stateprovider.Height(height/2))
			require.NoError(t, err)
...
func TestEdgeChallengeManager_BlockChallengeAddLevelZeroEdge(t *testing.T) {
	...
	require.NoError(t, err)
	prefixProof, err := createdData.HonestStateManager.PrefixProof(ctx, req, layer2stateprovider.Height(0))
	require.NoError(t, err)
...
func TestEdgeChallengeManager_Bisect(t *testing.T) {
	...
	t.Run("OK", func(t *testing.T) {
		req := &layer2stateprovider.HistoryCommitmentRequest{
			AssertionMetadata:           simpleAssertionMetadata(),
			UpperChallengeOriginHeights: []layer2stateprovider.Height{},
			UpToHeight:                  option.Some(layer2stateprovider.Height(challengetesting.LevelZeroBlockEdgeHeight / 2)),
		}
		honestBisectCommit, err := honestStateManager.HistoryCommitment(ctx, req)
		require.NoError(t, err)
		req.UpToHeight = option.Some(layer2stateprovider.Height(challengetesting.LevelZeroBlockEdgeHeight))
		honestProof, err := honestStateManager.PrefixProof(ctx, req, challengetesting.LevelZeroBlockEdgeHeight/2)
		require.NoError(t, err)
		lower, upper, err := honestEdge.Bisect(ctx, honestBisectCommit.Merkle, honestProof)
		require.NoError(t, err)
...
func TestEdgeChallengeManager_AddSubchallengeLeaf(t *testing.T) {
	...
	heights := &chainabstraction.LayerZeroHeights{
	...
	bisectionScenario := setupBisectionScenario(
		...
		setup.WithChallengeTestingOpts(
			challengetesting.WithLayerZeroHeights(heights),
			challengetesting.WithNumBigStepLevels(numBigSteps),
		),
	...
	var blockHeight uint64 = challengetesting.LevelZeroBlockEdgeHeight
	for blockHeight > 1 {
		bisectTo := layer2stateprovider.Height(blockHeight / 2)
		req := &layer2stateprovider.HistoryCommitmentRequest{
			AssertionMetadata:           simpleAssertionMetadata(),
			UpperChallengeOriginHeights: []layer2stateprovider.Height{},
		...
		req.UpToHeight = option.Some(layer2stateprovider.Height(blockHeight))
	...
	req := &layer2stateprovider.HistoryCommitmentRequest{
		AssertionMetadata:           simpleAssertionMetadata(),
		UpperChallengeOriginHeights: []layer2stateprovider.Height{0},
		UpToHeight:                  option.Some(layer2stateprovider.Height(0)),
	}
	startCommit, startErr := honestStateManager.HistoryCommitment(ctx, req)
	require.NoError(t, startErr)
	req.UpToHeight = option.None[layer2stateprovider.Height]()
	...
	req.UpToHeight = option.Some(layer2stateprovider.Height(1))
	...
	req = &layer2stateprovider.HistoryCommitmentRequest{
		AssertionMetadata:           simpleAssertionMetadata(),
		UpperChallengeOriginHeights: []layer2stateprovider.Height{0},
		UpToHeight:                  option.Some(layer2stateprovider.Height(endCommit.Height)),
	}
	...
	require.Equal(t, chainabstraction.ChallengeLevel(1), lvl)
}
...
func TestEdgeChallengeManager_ConfirmByOneStepProof(t *testing.T) {
	...
	t.Run("edge does not exist", func(t *testing.T) {
		...
		err := challengeManager.ConfirmEdgeByOneStepProof(
			ctx,
			chainabstraction.EdgeId{Hash: common.BytesToHash([]byte("foo"))},
			&chainabstraction.OneStepData{
	...
	t.Run("OK", func(t *testing.T) {
		...
		data, startInclusionProof, endInclusionProof, err := honestStateManager.OneStepProofData(
			...
			[]layer2stateprovider.Height{
				layer2stateprovider.Height(fromBlockChallengeHeight),
				layer2stateprovider.Height(fromBigStep),
			},
			layer2stateprovider.Height(smallStep),
		)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		require.Equal(t, chainabstraction.EdgeConfirmed, edgeStatus)
...
func TestEdgeChallengeManager_ConfirmByTime(t *testing.T) {
	...
	bisectTo := layer2stateprovider.Height(challengetesting.LevelZeroBlockEdgeHeight / 2)
	req := &layer2stateprovider.HistoryCommitmentRequest{
		AssertionMetadata:           simpleAssertionMetadata(),
		UpperChallengeOriginHeights: []layer2stateprovider.Height{},
		UpToHeight:                  option.Some(bisectTo),
	...
	require.NoError(t, err)
	require.Equal(t, chainabstraction.EdgePending, s1)
	s2, err := honestChildren2.Status(ctx)
	require.NoError(t, err)
	require.Equal(t, chainabstraction.EdgePending, s2)
	...
	_, err = chalManager.MultiUpdateInheritedTimers(ctx, []chainabstraction.ReadOnlyEdge{honestChildren1, honestChildren2, honestEdge}, expectedNewTimer)
	require.NoError(t, err)
	_, err = honestEdge.ConfirmByTimer(ctx, bisectionScenario.topLevelFork.Leaf1.Id())
	require.NoError(t, err)
	s0, err := honestEdge.Status(ctx)
	require.NoError(t, err)
	require.Equal(t, chainabstraction.EdgeConfirmed, s0)
	_, err = honestEdge.ConfirmByTimer(ctx, bisectionScenario.topLevelFork.Leaf1.Id())
	require.NoError(t, err)
}
...
func TestEdgeChallengeManager_ConfirmByTime_MoreComplexScenario(t *testing.T) {
	...
	s0, err := honestEdge.Status(ctx)
	require.NoError(t, err)
	require.Equal(t, chainabstraction.EdgePending, s0)
	...
	t.Run("confirmed by timer", func(t *testing.T) {
		...
		expectedNewTimer := uint64(200)
		_, err = chalManager.MultiUpdateInheritedTimers(ctx, []chainabstraction.ReadOnlyEdge{honestEdge}, expectedNewTimer)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		require.Equal(t, chainabstraction.EdgeConfirmed, status)
	})
...
type bisectionScenario struct {
	...
	honestStateManager  layer2stateprovider.Provider
	evilStateManager    layer2stateprovider.Provider
	honestLevelZeroEdge chainabstraction.VerifiedRoyalEdge
	evilLevelZeroEdge   chainabstraction.VerifiedRoyalEdge
...
func setupBisectionScenario(
	...
	// Honest assertion being added.
	leafAdder := func(stateManager layer2stateprovider.Provider, leaf chainabstraction.Assertion) (history.History, chainabstraction.VerifiedRoyalEdge) {
		req := &layer2stateprovider.HistoryCommitmentRequest{
			AssertionMetadata:           simpleAssertionMetadata(),
			UpperChallengeOriginHeights: []layer2stateprovider.Height{},
			UpToHeight:                  option.Some(layer2stateprovider.Height(0)),
...
type oneStepProofScenario struct {
	...
	honestStateManager  layer2stateprovider.Provider
	evilStateManager    layer2stateprovider.Provider
	smallStepHonestEdge chainabstraction.SpecEdge
	smallStepEvilEdge   chainabstraction.SpecEdge
}
...
func setupOneStepProofScenario(
	...
	bigStepAdder := func(stateManager layer2stateprovider.Provider, sourceEdge chainabstraction.SpecEdge) chainabstraction.VerifiedRoyalEdge {
		req := &layer2stateprovider.HistoryCommitmentRequest{
			AssertionMetadata:           simpleAssertionMetadata(),
			UpperChallengeOriginHeights: []layer2stateprovider.Height{0},
			UpToHeight:                  option.Some(layer2stateprovider.Height(0)),
	...
	var bigStepHeight uint64 = challengetesting.LevelZeroBigStepEdgeHeight
	for bigStepHeight > 1 {
		bisectTo := layer2stateprovider.Height(bigStepHeight / 2)
		...
		req.UpToHeight = option.Some(layer2stateprovider.Height(bigStepHeight))
	...
	smallStepAdder := func(stateManager layer2stateprovider.Provider, edge chainabstraction.SpecEdge) chainabstraction.VerifiedRoyalEdge {
		req := &layer2stateprovider.HistoryCommitmentRequest{
			AssertionMetadata:           simpleAssertionMetadata(),
			UpperChallengeOriginHeights: []layer2stateprovider.Height{0, 0},
			UpToHeight:                  option.Some(layer2stateprovider.Height(0)),
		}
		startCommit, startErr := stateManager.HistoryCommitment(ctx, req)
		require.NoError(t, startErr)
	...
	var smallStepHeight uint64 = challengetesting.LevelZeroBigStepEdgeHeight
	for smallStepHeight > 1 {
		bisectTo := layer2stateprovider.Height(smallStepHeight / 2)
		req := &layer2stateprovider.HistoryCommitmentRequest{
			AssertionMetadata:           simpleAssertionMetadata(),
			UpperChallengeOriginHeights: []layer2stateprovider.Height{0, 0},
			UpToHeight:                  option.Some(bisectTo),
		...
		req.UpToHeight = option.Some(layer2stateprovider.Height(smallStepHeight))
