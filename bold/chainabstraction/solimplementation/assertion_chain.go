...
// package solimplementation includes an easy-to-use abstraction
...
type ChainBackendTransactor struct {
	chainabstraction.ChainBackend
	fifo *FIFO
}

func NewChainBackendTransactor(backend chainabstraction.ChainBackend) *ChainBackendTransactor {
...
type AssertionChain struct {
	...
	confirmedChallengesByParentAssertionHash *threadsafe.LruSet[chainabstraction.AssertionHash]
...
func NewAssertionChain(
	...
	chain := &AssertionChain{
		...
		confirmedChallengesByParentAssertionHash: threadsafe.NewLruSet(1000, threadsafe.LruSetWithMetric[chainabstraction.AssertionHash]("confirmedChallengesByParentAssertionHash")),
...
func (a *AssertionChain) Backend() chainabstraction.ChainBackend {
	return a.backend
}
...
func (a *AssertionChain) GetAssertion(ctx context.Context, opts *bind.CallOpts, assertionHash chainabstraction.AssertionHash) (chainabstraction.Assertion, error) {
	var b [32]byte
	...
	if res.Status == uint8(chainabstraction.NoAssertion) {
...
func (a *AssertionChain) AssertionStatus(ctx context.Context, assertionHash chainabstraction.AssertionHash) (chainabstraction.AssertionStatus, error) {
	res, err := a.rollup.GetAssertion(&bind.CallOpts{Context: ctx}, assertionHash.Hash)
	if err != nil {
		return chainabstraction.NoAssertion, err
	}
	return chainabstraction.AssertionStatus(res.Status), nil
}

func (a *AssertionChain) LatestConfirmed(ctx context.Context, opts *bind.CallOpts) (chainabstraction.Assertion, error) {
	res, err := a.rollup.LatestConfirmed(opts)
	if err != nil {
		return nil, err
	}
	return a.GetAssertion(ctx, opts, chainabstraction.AssertionHash{Hash: res})
}
...
func (a *AssertionChain) IsChallengeComplete(
	ctx context.Context,
	challengeParentAssertionHash chainabstraction.AssertionHash,
) (bool, error) {
	...
	parentIsConfirmed := parentAssertionStatus == chainabstraction.AssertionConfirmed
...
func (a *AssertionChain) NewStakeOnNewAssertion(
	ctx context.Context,
	parentAssertionCreationInfo *chainabstraction.AssertionCreatedInfo,
...
func (a *AssertionChain) createAndStakeOnAssertion(
	...
	existingAssertion, err := a.GetAssertion(ctx, &bind.CallOpts{Context: ctx}, chainabstraction.AssertionHash{Hash: computedHash})
	switch {
	case err == nil:
	...
	if createErr := handleCreateAssertionError(err, postState.GlobalState.BlockHash); createErr != nil {
		if strings.Contains(createErr.Error(), "already exists") {
			assertionItem, err2 := a.GetAssertion(ctx, opts, chainabstraction.AssertionHash{Hash: computedHash})
			if err2 != nil {
				return nil, err2
			}
	...
	return a.GetAssertion(ctx, opts, chainabstraction.AssertionHash{Hash: assertionCreated.AssertionHash})
}
...
func TryConfirmingAssertion(
	...
	averageTimeForBlockCreation time.Duration,
	winningEdgeId option.Option[chainabstraction.EdgeId],
) (bool, error) {
	...
	if status == chainabstraction.NoAssertion {
		return false, fmt.Errorf("no assertion found by hash: %#x", assertionHash)
	}
	if status == chainabstraction.AssertionConfirmed {
		return true, nil
	}
	for {
	...
	if winningEdgeId.IsSome() {
		...
		if err != nil {
			if strings.Contains(err.Error(), chainabstraction.ChallengeGracePeriodNotPassedAssertionConfirmationError) {
				return false, nil
			}
		...
		if err != nil {
			if strings.Contains(err.Error(), chainabstraction.BeforeDeadlineAssertionConfirmationError) {
				return false, nil
			}
			if strings.Contains(err.Error(), "is not the latest confirmed assertion") {
				return false, nil
...
func (a *AssertionChain) ConfirmAssertionByTime(ctx context.Context, assertionHash chainabstraction.AssertionHash) error {
	return a.ConfirmAssertionByChallengeWinner(ctx, assertionHash, chainabstraction.EdgeId{})
}
...
func (a *AssertionChain) ConfirmAssertionByChallengeWinner(
	...
	if node.Status == uint8(chainabstraction.AssertionConfirmed) {
		return nil
	}
...
func (a *AssertionChain) SpecChallengeManager() chainabstraction.SpecChallengeManager {
...
func (a *AssertionChain) AssertionUnrivaledBlocks(ctx context.Context, assertionHash chainabstraction.AssertionHash) (uint64, error) {
	var b [32]byte
	copy(b[:], assertionHash.Bytes())
	...
	if wantNode.Status == uint8(chainabstraction.NoAssertion) {
	...
	if prevNode.Status == uint8(chainabstraction.NoAssertion) {
		return 0, errors.Wrapf(
			ErrNotFound,
			"assertion with id %#x",
			assertionHash,
...
func (a *AssertionChain) TopLevelAssertion(ctx context.Context, edgeId chainabstraction.EdgeId) (chainabstraction.AssertionHash, error) {
	cm := a.SpecChallengeManager()
	edgeOpt, err := cm.GetEdge(ctx, edgeId)
	if err != nil {
		return chainabstraction.AssertionHash{}, err
	}
	if edgeOpt.IsNone() {
		return chainabstraction.AssertionHash{}, errors.New("edge was nil")
	}
	return edgeOpt.Unwrap().AssertionHash(ctx)
}

func (a *AssertionChain) TopLevelClaimHeights(ctx context.Context, edgeId chainabstraction.EdgeId) (chainabstraction.OriginHeights, error) {
	cm := a.SpecChallengeManager()
	edgeOpt, err := cm.GetEdge(ctx, edgeId)
	if err != nil {
	...
	if edgeOpt.IsNone() {
		return chainabstraction.OriginHeights{}, errors.New("edge was nil")
	}
	edge := edgeOpt.Unwrap()
...
func (a *AssertionChain) ReadAssertionCreationInfo(
	...
	if id == (chainabstraction.AssertionHash{}) {
