...
import (
	...
	"github.com/offchainlabs/nitro/bold/challengemanager/challengetree"
...
func (e *specEdge) Id() chainabstraction.EdgeId {
	return chainabstraction.EdgeId{Hash: e.id}
}

func (e *specEdge) GetChallengeLevel() chainabstraction.ChallengeLevel {
	return chainabstraction.ChallengeLevel(e.inner.Level)
}
...
func (e *specEdge) GetReversedChallengeLevel() chainabstraction.ChallengeLevel {
	return chainabstraction.ChallengeLevel(e.totalChallengeLevels - 1 - e.inner.Level)
}
...
func (e *specEdge) StartCommitment() (chainabstraction.Height, common.Hash) {
	return chainabstraction.Height(e.startHeight), e.inner.StartHistoryRoot
}

func (e *specEdge) EndCommitment() (chainabstraction.Height, common.Hash) {
	return chainabstraction.Height(e.endHeight), e.inner.EndHistoryRoot
}

func (e *specEdge) AssertionHash(_ context.Context) (chainabstraction.AssertionHash, error) {
...
func (e *specEdge) Status(ctx context.Context) (chainabstraction.EdgeStatus, error) {
	if e.isConfirmed {
	...
	return chainabstraction.EdgeStatus(edge.Status), nil
}
...
func (e *specEdge) LowerChild(ctx context.Context) (option.Option[chainabstraction.EdgeId], error) {
	if e.lowerChild.IsSome() {
		return e.lowerChild, nil
	}
	edge, err := e.fetchEdge(ctx)
	if err != nil {
		return option.None[chainabstraction.EdgeId](), err
	}
	if edge.LowerChildId == ([32]byte{}) {
		return option.None[chainabstraction.EdgeId](), nil
	}
	return option.Some(chainabstraction.EdgeId{
		Hash: edge.LowerChildId,
	}), nil
}

// UpperChild of the edge, if any.
func (e *specEdge) UpperChild(ctx context.Context) (option.Option[chainabstraction.EdgeId], error) {
...
func (e *specEdge) MutualId() chainabstraction.MutualId {
	return e.mutualId
}

func (e *specEdge) OriginId() chainabstraction.OriginId {
	return e.inner.OriginId
}

// ClaimId of the edge, if any.
func (e *specEdge) ClaimId() option.Option[chainabstraction.ClaimId] {
	if e.inner.ClaimId == [32]byte{} {
		return option.None[chainabstraction.ClaimId]()
	}
	return option.Some(chainabstraction.ClaimId(e.inner.ClaimId))
}
...
func (e *specEdge) AsVerifiedHonest() (chainabstraction.VerifiedRoyalEdge, bool) {
	if e.verifiedHonest {
...
func (h *honestEdge) Bisect(
	...
	var upperEdge option.Option[chainabstraction.SpecEdge]
	var lowerId option.Option[chainabstraction.EdgeId]
	var lowerEdge option.Option[chainabstraction.SpecEdge]
	if !upperId.IsNone() {
	...
	someEdge, err := e.manager.GetEdge(ctx, chainabstraction.EdgeId{Hash: e.id})
	if err != nil {
		return nil, nil, err
	}
	if someEdge.IsNone() {
	...
	// Refresh the edge.
	e = edge
	someLowerChild, err := e.manager.GetEdge(ctx, chainabstraction.EdgeId{Hash: e.inner.LowerChildId})
	if err != nil {
		return nil, nil, err
	}
	someUpperChild, err := e.manager.GetEdge(ctx, chainabstraction.EdgeId{Hash: e.inner.UpperChildId})
	if err != nil {
		return nil, nil, err
	}
...
func (h *honestEdge) ConfirmByTimer(ctx context.Context, claimedAssertion chainabstraction.AssertionHash) (*types.Transaction, error) {
	e := h.specEdge
	s, err := e.Status(ctx)
	if err != nil {
		return nil, err
	}
	if s == chainabstraction.EdgeConfirmed {
		return nil, nil
	}
...
// is the height of A.
func (e *specEdge) TopLevelClaimHeight(ctx context.Context) (chainabstraction.OriginHeights, error) {
	challengeLevel := e.GetChallengeLevel()
	if challengeLevel == 0 {
		startHeight, _ := e.StartCommitment()
		return chainabstraction.OriginHeights{
			ChallengeOriginHeights: []chainabstraction.Height{startHeight},
		}, nil
	}
	challengeOriginHeights := make([]chainabstraction.Height, challengeLevel)
	originId := e.inner.OriginId
	for challengeLevel > 0 {
		if ctx.Err() != nil {
			return chainabstraction.OriginHeights{}, ctx.Err()
		}
		...
		if err != nil {
			return chainabstraction.OriginHeights{}, err
		}
		challengeOneStepForkSource, err := e.manager.GetEdge(ctx, chainabstraction.EdgeId{Hash: rivalId})
		if err != nil {
			return chainabstraction.OriginHeights{}, errors.Wrapf(err, "big step challenge one step fork source does not exist: origin id %#x, rival %#x, challenge level %d", originId, rivalId, challengeLevel)
		}
		if challengeOneStepForkSource.IsNone() {
			return chainabstraction.OriginHeights{}, errors.New("source edge is none")
		}
		bigStepEdge, ok := challengeOneStepForkSource.Unwrap().(*specEdge)
		if !ok {
			return chainabstraction.OriginHeights{}, errors.New("not *SpecEdge")
		}
...
type specChallengeManager struct {
	...
	layerZeroHeights      *chainabstraction.LayerZeroHeights
...
func NewSpecChallengeManager(
	...
	backend chainabstraction.ChainBackend,
	txOpts *bind.TransactOpts,
) (chainabstraction.SpecChallengeManager, error) {
...
func (cm *specChallengeManager) LayerZeroHeights() chainabstraction.LayerZeroHeights {
	return *cm.layerZeroHeights
}

func lookupLayerZeroHeights(ctx context.Context, chain *AssertionChain, caller challengeV2gen.EdgeChallengeManagerCaller) (*chainabstraction.LayerZeroHeights, error) {
	...
	return &chainabstraction.LayerZeroHeights{
		BlockChallengeHeight:     chainabstraction.Height(h.Uint64()),
		BigStepChallengeHeight:   chainabstraction.Height(bs.Uint64()),
		SmallStepChallengeHeight: chainabstraction.Height(ss.Uint64()),
	}, nil
}
...
func (cm *specChallengeManager) GetEdge(
	...
	if err != nil {
		return option.None[chainabstraction.SpecEdge](), err
	}
	...
	if !edge.StartHeight.IsUint64() {
		return option.None[chainabstraction.SpecEdge](), errors.New("start height not a uint64")
	}
	if !edge.EndHeight.IsUint64() {
		return option.None[chainabstraction.SpecEdge](), errors.New("end height not a uint64")
	}
	...
	if err != nil {
		return option.Option[chainabstraction.SpecEdge]{}, err
	}
	return option.Some(chainabstraction.SpecEdge(&specEdge{
		id:                   edgeId.Hash,
		mutualId:             mutual,
		manager:              cm,
		inner:                edge,
		...
		assertionHash:        chainabstraction.AssertionHash{Hash: common.Hash(assertionHash)},
	})), nil
}

func (e *specEdge) LatestInheritedTimer(ctx context.Context) (chainabstraction.InheritedTimer, error) {
	...
	if edgetracker.IsRootBlockChallengeEdge(e) {
		...
		assertionUnrivaledBlocks, err := e.manager.assertionChain.AssertionUnrivaledBlocks(
			ctx,
			chainabstraction.AssertionHash{Hash: common.Hash(e.ClaimId().Unwrap())},
		)
		if err != nil {
			return 0, err
		}
		return chainabstraction.InheritedTimer(edge.TotalTimeUnrivaledCache + assertionUnrivaledBlocks), nil
	}
	return chainabstraction.InheritedTimer(edge.TotalTimeUnrivaledCache), nil
}
...
func (e *specEdge) fetchEdge(
	...
	if chainabstraction.EdgeStatus(edge.Status) == chainabstraction.EdgeConfirmed {
		e.isConfirmed = true
	}
	...
	if edge.LowerChildId != ([32]byte{}) {
		e.lowerChild = option.Some(chainabstraction.EdgeId{Hash: edge.LowerChildId})
	}
	if edge.UpperChildId != ([32]byte{}) {
		e.upperChild = option.Some(chainabstraction.EdgeId{Hash: edge.UpperChildId})
	}
	return edge, nil
}
...
func (cm *specChallengeManager) CalculateEdgeId(
	...
	if err != nil {
		return chainabstraction.EdgeId{}, err
	}
	...
	return chainabstraction.EdgeId{Hash: id}, err
}

func (cm *specChallengeManager) MultiUpdateInheritedTimers(
	ctx context.Context,
	challengeBranch []chainabstraction.ReadOnlyEdge,
...
func (cm *specChallengeManager) AddBlockChallengeLevelZeroEdge(
	...
	edgeId, err := cm.CalculateEdgeId(
		...
		chainabstraction.OriginId(assertionCreation.ParentAssertionHash.Hash),
		chainabstraction.Height(startCommit.Height),
		startCommit.Merkle,
		chainabstraction.Height(endCommit.Height),
		endCommit.Merkle,
	)
	if err != nil {
	...
	args := challengeV2gen.CreateEdgeArgs{
		Level:          chainabstraction.NewBlockChallengeLevel().Uint8(),
	...
	someLevelZeroEdge, err = cm.GetEdge(ctx, chainabstraction.EdgeId{Hash: edgeAdded.EdgeId})
	if err != nil {
...
func (cm *specChallengeManager) AddSubChallengeLevelZeroEdge(
	...
	edgeId, err := cm.CalculateEdgeId(
		ctx,
		subChalTyp,
		chainabstraction.OriginId(mutualId),
		chainabstraction.Height(startCommit.Height),
		startCommit.Merkle,
		chainabstraction.Height(endCommit.Height),
		endCommit.Merkle,
