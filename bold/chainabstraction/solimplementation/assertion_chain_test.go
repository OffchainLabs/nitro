...
func TestNewStakeOnNewAssertion(t *testing.T) {
	...
	require.NoError(t, err)
	genesisInfo, err := chain.ReadAssertionCreationInfo(ctx, chainabstraction.AssertionHash{Hash: genesisHash})
	require.NoError(t, err)
...
func TestStakeOnNewAssertion(t *testing.T) {
	...
	gotPostState := chainabstraction.GoExecutionStateFromSolidity(newAssertionCreatedInfo.AfterState)
...
func TestConfirmAssertionByChallengeWinner(t *testing.T) {
	...
	leafAdder := func(stateManager layer2stateprovider.Provider, leaf chainabstraction.Assertion) chainabstraction.VerifiedRoyalEdge {
		...
		prefixProof, proofErr := stateManager.PrefixProof(ctx, req, layer2stateprovider.Height(0))
	...
	t.Run("genesis case", func(t *testing.T) {
		err = chain.ConfirmAssertionByChallengeWinner(
			ctx, latestConfirmed.Id(), chainabstraction.EdgeId{},
		)
		require.NoError(t, err)
	})
...
func TestAssertionBySequenceNum(t *testing.T) {
	...
	_, err = chain.GetAssertion(ctx, opts, chainabstraction.AssertionHash{Hash: common.BytesToHash([]byte("foo"))})
	require.ErrorIs(t, err, solimplementation.ErrNotFound)
}
...
func Test_autoDepositFunds_SkipsIfAlreadyStaked(t *testing.T) {
	...
	require.NoError(t, err)
	genesisInfo, err := assertionChain.ReadAssertionCreationInfo(ctx, chainabstraction.AssertionHash{Hash: genesisHash})
	require.NoError(t, err)
	postState := &chainabstraction.ExecutionState{
		GlobalState: chainabstraction.GoGlobalState{
			BlockHash:  common.BytesToHash([]byte("foo")),
