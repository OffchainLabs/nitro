...
func (cc *challengeConfirmer) beginConfirmationJob(
	...
	royalRootEdge chainabstraction.VerifiedRoyalEdge,
	...
	royalTreeLeaves, err := retry.UntilSucceeds(ctx, func() ([]chainabstraction.SpecEdge, error) {
	...
	royalBranches := make([][]chainabstraction.ReadOnlyEdge, 0)
	for _, edge := range royalTreeLeaves {
		branch := []chainabstraction.ReadOnlyEdge{edge}
		ancestors, err2 := retry.UntilSucceeds(ctx, func() ([]chainabstraction.ReadOnlyEdge, error) {
	...
	onchainInheritedTimer, err := retry.UntilSucceeds(ctx, func() (chainabstraction.InheritedTimer, error) {
	...
	if onchainInheritedTimer < chainabstraction.InheritedTimer(challengePeriodBlocks) {
...
func (cc *challengeConfirmer) propageTimerUpdateToBranch(
	...
	branchIdx,
	totalBranches int,
	branch []chainabstraction.ReadOnlyEdge,
	challengePeriodBlocks uint64,
	claimedAssertionHash chainabstraction.AssertionHash,
) (*types.Transaction, error) {
	if len(branch) == 0 {
	...
	tx, err := retry.UntilSucceeds(ctx, func() (*types.Transaction, error) {
		...
		if innerErr != nil {
			...
			if strings.Contains(innerErr.Error(), chainabstraction.ErrCachedTimeSufficient) {
	...
	rootTimer, err := retry.UntilSucceeds(ctx, func() (chainabstraction.InheritedTimer, error) {
		timer, innerErr := royalRootEdge.LatestInheritedTimer(ctx)
		if innerErr != nil {
			fields = append(fields, "err", innerErr)
			log.Error("Could not get inherited timer for edge", fields...)
