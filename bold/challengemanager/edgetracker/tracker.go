...
type HonestChallengeTreeWriter interface {
	...
	AddVerifiedHonestEdge(
		ctx context.Context, verifiedHonest chainabstraction.VerifiedRoyalEdge,
	) error
}
...
type ChallengeTracker interface {
	IsTrackingEdge(chainabstraction.EdgeId) bool
	MarkTrackedEdge(chainabstraction.EdgeId, *Tracker)
	RemovedTrackedEdge(chainabstraction.EdgeId)
...
func WithTimeReference(ref timereference.Reference) Opt {
...
type Tracker struct {
	...
	stateProvider               layer2stateprovider.Provider
	chainWatcher                HonestChallengeTreeWriter
	challengeManager            ChallengeTracker
	associatedAssertionMetadata *layer2stateprovider.AssociatedAssertionMetadata
...
func New(
	...
	chainWatcher HonestChallengeTreeWriter,
	challengeManager ChallengeTracker,
	assertionCreationInfo *layer2stateprovider.AssociatedAssertionMetadata,
	opts ...Opt,
) (*Tracker, error) {
	tr := &Tracker{
		edge:                        edge,
...
func (et *Tracker) AssertionInfo() *layer2stateprovider.AssociatedAssertionMetadata {
	return et.associatedAssertionMetadata
}

func (et *Tracker) EdgeId() chainabstraction.EdgeId {
	return et.edge.Id()
}
...
func (et *Tracker) ShouldDespawn(ctx context.Context) bool {
	...
	if et.edge.ClaimId().IsSome() {
		...
		return status == chainabstraction.EdgeConfirmed
	}
...
func (et *Tracker) tryToConfirmEdge(ctx context.Context) (bool, error) {
	...
	if status == chainabstraction.EdgeConfirmed {
		return true, nil
	}
...
func (et *Tracker) DetermineBisectionHistoryWithProof(
	...
	if challengeLevel == chainabstraction.NewBlockChallengeLevel() {
		historyCommit, commitErr := et.stateProvider.HistoryCommitment(
			...
			&layer2stateprovider.HistoryCommitmentRequest{
				...
				UpToHeight:                  option.Some(layer2stateprovider.Height(bisectTo)),
			},
		)
		if commitErr != nil {
		...
		proof, proofErr := et.stateProvider.PrefixProof(
			...
			layer2stateprovider.Height(bisectTo),
		)
		if proofErr != nil {
	...
	challengeOriginHeights := make([]layer2stateprovider.Height, len(originHeights.ChallengeOriginHeights))
	for index, height := range originHeights.ChallengeOriginHeights {
		challengeOriginHeights[index] = layer2stateprovider.Height(height)
	}
...
func (et *Tracker) bisect(ctx context.Context) (chainabstraction.VerifiedRoyalEdge, chainabstraction.VerifiedRoyalEdge, error) {
...
func (et *Tracker) openSubchallenge(ctx context.Context) error {
	...
	fromBlockChallengeHeight := layer2stateprovider.Height(originHeights.ChallengeOriginHeights[0])
	...
	case chainabstraction.NewBlockChallengeLevel():
		endHistory, err = et.stateProvider.HistoryCommitment(
			ctx,
			&layer2stateprovider.HistoryCommitmentRequest{
				AssertionMetadata:           et.associatedAssertionMetadata,
				UpperChallengeOriginHeights: []layer2stateprovider.Height{fromBlockChallengeHeight},
				UpToHeight:                  option.None[layer2stateprovider.Height](),
			},
		)
		if err != nil {
		...
		startEndPrefixProof, err = et.stateProvider.PrefixProof(
			...
			&layer2stateprovider.HistoryCommitmentRequest{
				AssertionMetadata:           et.associatedAssertionMetadata,
				UpperChallengeOriginHeights: []layer2stateprovider.Height{fromBlockChallengeHeight},
				UpToHeight:                  option.Some(layer2stateprovider.Height(endHistory.Height)),
			},
			layer2stateprovider.Height(0),
		)
		if err != nil {
	...
	default:
		heights := make([]layer2stateprovider.Height, 0)
		for _, h := range originHeights.ChallengeOriginHeights {
			heights = append(heights, layer2stateprovider.Height(h))
		}
		heights = append(heights, layer2stateprovider.Height(startHeight))
		...
		request = &layer2stateprovider.HistoryCommitmentRequest{
			AssertionMetadata:           et.associatedAssertionMetadata,
			UpperChallengeOriginHeights: heights[:len(heights)-1],
			UpToHeight:                  option.Some(layer2stateprovider.Height(startHeight)),
		}
		startParentCommitment, err = et.stateProvider.HistoryCommitment(
			ctx,
			request,
...
func canOneStepProve(ctx context.Context, edge chainabstraction.SpecEdge) (bool, error) {
...
func IsRootBlockChallengeEdge(edge chainabstraction.ReadOnlyEdge) bool {
	return edge.ClaimId().IsSome() && edge.GetChallengeLevel() == chainabstraction.NewBlockChallengeLevel()
}
