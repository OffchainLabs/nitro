...
func Test_getEdgeTrackers(t *testing.T) {
	...
	edge.On("Id").Return(chainabstraction.EdgeId{Hash: common.BytesToHash([]byte("foo"))})
	edge.On("GetReversedChallengeLevel").Return(chainabstraction.ChallengeLevel(2))
	...
	parentAssertionHash := chainabstraction.AssertionHash{Hash: common.BytesToHash([]byte("par"))}
	...
	edge.On("StartCommitment").Return(chainabstraction.Height(0), common.Hash{})
	edge.On("EndCommitment").Return(chainabstraction.Height(0), common.Hash{})
	edge.On("GetChallengeLevel").Return(chainabstraction.ChallengeLevel(0))
	edge.On("MarkAsHonest").Return()
	edge.On("AsVerifiedHonest").Return(honest, true)
	m.On("ReadAssertionCreationInfo", ctx, assertionHash).Return(&chainabstraction.AssertionCreatedInfo{
	...
	m.On("ReadAssertionCreationInfo", ctx, parentAssertionHash).Return(&chainabstraction.AssertionCreatedInfo{
		InboxMaxCount: big.NewInt(100),
	}, nil)
	s.On("ExecutionStateMsgCount", ctx, &chainabstraction.ExecutionState{}).Return(uint64(1), nil)
	...
	require.Equal(t, layer2stateprovider.Batch(1), layer2stateprovider.Batch(trk.AssertionInfo().FromState.Batch))
	require.Equal(t, layer2stateprovider.Batch(100), trk.AssertionInfo().BatchLimit)
}
...
func setupEdgeTrackersForBisection(
	...
	assertionInfo := &layer2stateprovider.AssociatedAssertionMetadata{
		FromState:            chainabstraction.GoGlobalState{Batch: 0, PosInBatch: 0},
		BatchLimit:           1,
	...
	tracker1, err := edgetracker.New(
		...
		honestValidator,
		assertionInfo,
		edgetracker.WithTimeReference(timereference.NewArtificialTimeReference()),
