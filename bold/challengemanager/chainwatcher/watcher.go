...
//
// See: [github.com/offchainlabs/nitro/bold/challengemanager/edgetracker]
...
type EdgeManager interface {
	TrackEdge(ctx context.Context, edge chainabstraction.VerifiedRoyalEdge) error
}
...
type trackedChallenge struct {
	...
	confirmedLevelZeroEdgeClaimIds *threadsafe.Map[chainabstraction.ClaimId, chainabstraction.EdgeId]
}
...
type Watcher struct {
	...
	challenges                  *threadsafe.Map[chainabstraction.AssertionHash, *trackedChallenge]
	...
	evilEdgesByLevel            *threadsafe.Map[chainabstraction.ChallengeLevel, *threadsafe.Set[chainabstraction.EdgeId]]
	// Only track challenges for these parent assertion hashes.
	// Track all if empty / nil.
	trackChallengeParentAssertionHashes []chainabstraction.AssertionHash
...
func New(
	...
	return &Watcher{
		...
		challenges:                          threadsafe.NewMap(threadsafe.MapWithMetric[chainabstraction.AssertionHash, *trackedChallenge]("challenges")),
		...
		evilEdgesByLevel:                    threadsafe.NewMap(threadsafe.MapWithMetric[chainabstraction.ChallengeLevel, *threadsafe.Set[chainabstraction.EdgeId]]("evilEdgesByLevel")),
...
) (chainabstraction.ReadOnlyEdge, error) {
...
func (w *Watcher) ConfirmedEdgeWithClaimExists(
	...
	claimId chainabstraction.ClaimId,
) (chainabstraction.EdgeId, bool) {
	challenge, ok := w.challenges.TryGet(topLevelAssertionHash)
	if !ok {
		return chainabstraction.EdgeId{}, false
	}
	return challenge.confirmedLevelZeroEdgeClaimIds.TryGet(claimId)
}

func (w *Watcher) IsRoyal(assertionHash chainabstraction.AssertionHash, edgeId chainabstraction.EdgeId) bool {
	chal, ok := w.challenges.TryGet(assertionHash)
	if !ok {
		return false
	}
...
) (chainabstraction.InheritedTimer, error) {
...
// hash.
func (w *Watcher) GetRoyalEdges(ctx context.Context) (map[chainabstraction.AssertionHash][]*api.JsonTrackedRoyalEdge, error) {
	l1BlockNum, err := w.chain.DesiredL1HeaderU64(ctx)
	if err != nil {
		return nil, err
	}
	response := make(map[chainabstraction.AssertionHash][]*api.JsonTrackedRoyalEdge)
	if err = w.challenges.ForEach(func(assertionHash chainabstraction.AssertionHash, t *trackedChallenge) error {
		return t.honestEdgeTree.GetEdges().ForEach(func(edgeId chainabstraction.EdgeId, edge chainabstraction.SpecEdge) error {
...
func (w *Watcher) BlockChallengeRootEdge(
	ctx context.Context,
	challengedAssertionHash chainabstraction.AssertionHash,
) (chainabstraction.SpecEdge, error) {
...
func (w *Watcher) ClosestEssentialAncestor(
	ctx context.Context,
	challengedAssertionHash chainabstraction.AssertionHash,
	edge chainabstraction.VerifiedRoyalEdge,
) (chainabstraction.ReadOnlyEdge, error) {
	chal, ok := w.challenges.TryGet(challengedAssertionHash)
	if !ok {
		return nil, fmt.Errorf(
...
func (w *Watcher) IsEssentialAncestorConfirmable(
	ctx context.Context,
	edge chainabstraction.SpecEdge,
	challengedAssertionHash chainabstraction.AssertionHash,
...
func (w *Watcher) IsConfirmableEssentialEdge(
	ctx context.Context,
	challengedAssertionHash chainabstraction.AssertionHash,
	essentialEdgeId chainabstraction.EdgeId,
	confirmationThreshold uint64,
...
func (w *Watcher) AllowTrackingEdgeWithParentHash(parentHash chainabstraction.AssertionHash) bool {
...
func (w *Watcher) AddVerifiedHonestEdge(ctx context.Context, edge chainabstraction.VerifiedRoyalEdge) error {
	...
	if !ok {
		...
		chal = &trackedChallenge{
			honestEdgeTree:                 tree,
			confirmedLevelZeroEdgeClaimIds: threadsafe.NewMap(threadsafe.MapWithMetric[chainabstraction.ClaimId, chainabstraction.EdgeId]("confirmedLevelZeroEdgeClaimIds")),
		}
...
func (w *Watcher) AddEdge(ctx context.Context, edge chainabstraction.SpecEdge) (bool, error) {
	...
	if isRoyal {
		...
		if edge.ClaimId().IsSome() {
			...
			if !ok {
				evilEdges = threadsafe.NewSet(threadsafe.SetWithMetric[chainabstraction.EdgeId]("evilEdges"))
...
func (w *Watcher) processEdgeAddedEvent(
	...
	challengeManager := w.chain.SpecChallengeManager()
	edgeOpt, err := challengeManager.GetEdge(ctx, chainabstraction.EdgeId{Hash: event.EdgeId})
	if err != nil {
		return false, err
	}
	if edgeOpt.IsNone() {
...
func (w *Watcher) allowTrackingEdgeWithChallengeParentAssertionHash(challengeParentAssertionHash chainabstraction.AssertionHash) bool {
	if len(w.trackChallengeParentAssertionHashes) == 0 {
		return true
	}
	for _, hash := range w.trackChallengeParentAssertionHashes {
...
func (w *Watcher) checkForEdgeConfirmedByOneStepProof(
	...
	for it.Next() {
		...
		_, processErr := retry.UntilSucceeds(ctx, func() (bool, error) {
			return true, w.processEdgeConfirmation(ctx, chainabstraction.EdgeId{
...
func (w *Watcher) processEdgeConfirmation(
	...
	if challengeLevel == chainabstraction.NewBlockChallengeLevel() {
		claimedAssertion := chainabstraction.AssertionHash{Hash: common.Hash(claimId)}
...
func (w *Watcher) confirmAssertionByChallengeWinner(ctx context.Context, edge chainabstraction.SpecEdge, claimedAssertion chainabstraction.AssertionHash, challengeParentAssertionHash chainabstraction.AssertionHash) {
	...
	assertionCreationInfo, err := retry.UntilSucceeds(ctx, func() (*chainabstraction.AssertionCreatedInfo, error) {
	...
	parentCreationInfo, err := retry.UntilSucceeds(ctx, func() (*chainabstraction.AssertionCreatedInfo, error) {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ticker.C:
			confirmed, err := solimplementation.TryConfirmingAssertion(
				ctx,
				claimedAssertion,
				confirmableAtBlock,
				w.chain,
...
func (w *Watcher) getStartEndBlockNum(ctx context.Context) (filterRange, error) {
	latestConfirmedAssertion, err := retry.UntilSucceeds(ctx, func() (chainabstraction.Assertion, error) {
