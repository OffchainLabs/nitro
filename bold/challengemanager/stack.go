...
func StackWithTrackChallengeParentAssertionHashes(hashes []string) StackOpt {
	return func(p *stackParams) {
		p.trackChallengeParentAssertionHashes = make([]chainabstraction.AssertionHash, len(hashes))
		for i, h := range hashes {
			p.trackChallengeParentAssertionHashes[i] = chainabstraction.AssertionHash{Hash: common.HexToHash(h)}
		}
	}
}
...
func NewChallengeStack(
	...
	provider layer2stateprovider.Provider,
	opts ...StackOpt,
) (*Manager, error) {
