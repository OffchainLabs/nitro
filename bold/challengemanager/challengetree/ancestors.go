...
type HonestAncestors []chainabstraction.ReadOnlyEdge
...
// to any number of challenge levels in the chainabstraction.
func (ht *RoyalChallengeTree) ComputeAncestors(
	...
	ancestry := make([]chainabstraction.ReadOnlyEdge, 0)
	...
	for currentChallengeLevel < chainabstraction.ChallengeLevel(ht.totalChallengeLevels) {
		...
		if currentChallengeLevel == chainabstraction.ChallengeLevel(ht.totalChallengeLevels) {
			break
		}
...
func (ht *RoyalChallengeTree) ClosestEssentialAncestor(
	ctx context.Context, child chainabstraction.ReadOnlyEdge,
	...
	var claimedEdge chainabstraction.SpecEdge
	var ok bool
	_ = ht.edges.ForEach(func(_ chainabstraction.EdgeId, edge chainabstraction.SpecEdge) error {
		if edge.MutualId() == chainabstraction.MutualId(childOrigin) {
			claimedEdge = edge
			ok = true
		}
		return nil
...
func (ht *RoyalChallengeTree) findHonestAncestorsWithinChallengeLevel(
	ctx context.Context,
	rootEdge chainabstraction.ReadOnlyEdge,
	queryingFor chainabstraction.ReadOnlyEdge,
	blockNumber uint64,
) ([]EdgeLocalTimer, []chainabstraction.ReadOnlyEdge, error) {
	found := false
	cursor := rootEdge
	ancestry := make([]chainabstraction.ReadOnlyEdge, 0)
...
func (ht *RoyalChallengeTree) hasHonestAncestry(ctx context.Context, eg chainabstraction.SpecEdge) (bool, error) {
	chalLevel := eg.GetChallengeLevel()
	claimId := eg.ClaimId()

	// If the edge is a root edge at the block challenge level, then we return true early.
	if chalLevel == chainabstraction.NewBlockChallengeLevel() && claimId.IsSome() {
		return true, nil
	}
...
func (ht *RoyalChallengeTree) honestRootAncestorAtChallengeLevel(
	childEdge chainabstraction.ReadOnlyEdge,
...
func (ht *RoyalChallengeTree) getClaimedEdge(edge chainabstraction.ReadOnlyEdge) (chainabstraction.SpecEdge, error) {
	...
	claimedBlockEdge, ok := ht.edges.TryGet(chainabstraction.EdgeId{Hash: claimIdHash})
	if !ok {
...
func findOriginEdge(originId chainabstraction.OriginId, edges *threadsafe.Slice[chainabstraction.SpecEdge]) (chainabstraction.SpecEdge, bool) {
	var originEdge chainabstraction.SpecEdge
	found := edges.Find(func(_ int, e chainabstraction.SpecEdge) bool {
...
func errNotFound(id chainabstraction.EdgeId) error {
