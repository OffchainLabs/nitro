...
func (ht *RoyalChallengeTree) LocalTimer(ctx context.Context, e chainabstraction.ReadOnlyEdge, blockNum uint64) (uint64, error) {
...
func (ht *RoyalChallengeTree) EarliestCreatedRivalBlockNumber(e chainabstraction.ReadOnlyEdge) option.Option[uint64] {
...
func (ht *RoyalChallengeTree) UnrivaledAtBlockNum(e chainabstraction.ReadOnlyEdge, blockNum uint64) (bool, error) {
	createdAtBlock, err := e.CreatedAtBlock()
	if err != nil {
		return false, err
	}
...
func (ht *RoyalChallengeTree) rivalsWithCreationTimes(eg chainabstraction.ReadOnlyEdge) []*rival {
	rivals := make([]*rival, 0)
	...
	_ = mutuals.ForEach(func(rivalId chainabstraction.EdgeId, t creationTime) error {
		if rivalId == eg.Id() {
			return nil
		}
