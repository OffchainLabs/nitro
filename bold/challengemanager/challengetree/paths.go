...
type ComputePathWeightArgs struct {
	Child    chainabstraction.EdgeId
	Ancestor chainabstraction.EdgeId
	BlockNum uint64
}
...
type EssentialPath []chainabstraction.EdgeId
...
func (ht *RoyalChallengeTree) findEssentialPaths(
	ctx context.Context,
	essentialEdge chainabstraction.ReadOnlyEdge,
	blockNum uint64,
...
func (ht *RoyalChallengeTree) isClaimedEdge(ctx context.Context, edge chainabstraction.ReadOnlyEdge) (bool, chainabstraction.ReadOnlyEdge) {
...
func IsClaimingAnEdge(edge chainabstraction.ReadOnlyEdge) bool {
	return edge.ClaimId().IsSome() && edge.GetChallengeLevel() != chainabstraction.NewBlockChallengeLevel()
}

func hasLengthOne(edge chainabstraction.ReadOnlyEdge) bool {
...
func isProofEdge(ctx context.Context, edge chainabstraction.ReadOnlyEdge) bool {
	isSmallStep := edge.GetChallengeLevel() == chainabstraction.ChallengeLevel(edge.GetTotalChallengeLevels(ctx)-1)
