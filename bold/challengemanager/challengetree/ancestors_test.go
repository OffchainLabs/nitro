...
func TestClosestEssentialAncestor(t *testing.T) {
	...
	tree.royalRootEdgesByLevel.Put(2, threadsafe.NewSlice[chainabstraction.SpecEdge]())
	tree.royalRootEdgesByLevel.Put(1, threadsafe.NewSlice[chainabstraction.SpecEdge]())
	tree.royalRootEdgesByLevel.Put(0, threadsafe.NewSlice[chainabstraction.SpecEdge]())
...
func Test_findOriginEdge(t *testing.T) {
	edges := threadsafe.NewSlice[chainabstraction.SpecEdge]()
	origin := chainabstraction.OriginId(common.BytesToHash([]byte("foo")))
	...
	origin = chainabstraction.OriginId(common.BytesToHash([]byte("bar")))
	got, ok := findOriginEdge(origin, edges)
	require.Equal(t, true, ok)
	require.Equal(t, got.Id(), chainabstraction.EdgeId{Hash: common.BytesToHash([]byte("blk-0.a-4.a"))})
	...
	origin = chainabstraction.OriginId(common.BytesToHash([]byte("baz")))
	got, ok = findOriginEdge(origin, edges)
	require.Equal(t, true, ok)
	require.Equal(t, got.Id(), chainabstraction.EdgeId{Hash: common.BytesToHash([]byte("blk-0.b-4.b"))})
}
...
func setupBlockChallengeTreeSnapshot(t *testing.T, tree *RoyalChallengeTree, claimId string) {
	...
	transformedEdges := make(map[chainabstraction.EdgeId]chainabstraction.SpecEdge)
	...
	key := buildEdgeCreationTimeKey(chainabstraction.OriginId{}, mutual)
	tree.edgeCreationTimes.Put(key, threadsafe.NewMap[chainabstraction.EdgeId, creationTime]())
...
func id(eId mock.EdgeId) chainabstraction.EdgeId {
	return chainabstraction.EdgeId{Hash: common.BytesToHash([]byte(eId))}
}
...
func setupBigStepChallengeSnapshot(t *testing.T, tree *RoyalChallengeTree, claimId string) {
	...
	originId := chainabstraction.OriginId(common.BytesToHash([]byte(originEdge.ComputeMutualId())))
	aliceEdges := buildEdges(
		// Alice.
