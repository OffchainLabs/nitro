...
import (
	...
	"github.com/offchainlabs/nitro/bold/util/timereference"
...
type Manager struct {
	...
	chain                        chainabstraction.Protocol
	assertionManager             AssertionManager
	watcher                      *chainwatcher.Watcher
	stateManager                 layer2stateprovider.Provider
	name                         string
	headerProvider               HeaderProvider
	timeRef                      timereference.Reference
	trackedEdgeIds               *threadsafe.Map[chainabstraction.EdgeId, *edgetracker.Tracker]
	assertionMetadataCache       *threadsafe.LruMap[chainabstraction.AssertionHash, layer2stateprovider.AssociatedAssertionMetadata]
	newBlockNotifier             *events.Producer[*gethtypes.Header]
	notifyOnNumberOfBlocks       uint64
	mode                         types.Mode
	claimedAssertionsInChallenge *threadsafe.LruSet[chainabstraction.AssertionHash]
	// API
	api *server.Server
}
...
func New(
	...
	m := &Manager{
		...
		assertionManager:             assertionManager,
		watcher:                      watcher,
		timeRef:                      timereference.NewRealTimeReference(),
		trackedEdgeIds:               threadsafe.NewMap(threadsafe.MapWithMetric[chainabstraction.EdgeId, *edgetracker.Tracker]("trackedEdgeIds")),
		assertionMetadataCache:       threadsafe.NewLruMap(maxAssertions, threadsafe.LruMapWithMetric[chainabstraction.AssertionHash, layer2stateprovider.AssociatedAssertionMetadata]("batchIndexForAssertionCache")),
		notifyOnNumberOfBlocks:       1,
		newBlockNotifier:             events.NewProducer[*gethtypes.Header](),
		claimedAssertionsInChallenge: threadsafe.NewLruSet(maxAssertions, threadsafe.LruSetWithMetric[chainabstraction.AssertionHash]("claimedAssertionsInChallenge")),
		api:                          nil,
	}
	for _, o := range opts {
		o(m)
...
func (m *Manager) GetEdgeTracker(edgeId chainabstraction.EdgeId) option.Option[*edgetracker.Tracker] {
...
func (m *Manager) IsTrackingEdge(edgeId chainabstraction.EdgeId) bool {
	return m.trackedEdgeIds.Has(edgeId)
}

// MarkTrackedEdge marks an edge id as being tracked by our challenge manager.
func (m *Manager) MarkTrackedEdge(edgeId chainabstraction.EdgeId, tracker *edgetracker.Tracker) {
	m.trackedEdgeIds.Put(edgeId, tracker)
}

func (m *Manager) RemovedTrackedEdge(edgeId chainabstraction.EdgeId) {
...
// challenge.
func (m *Manager) IsClaimedByChallenge(assertionHash chainabstraction.AssertionHash) bool {
...
func (m *Manager) TrackEdge(ctx context.Context, edge chainabstraction.VerifiedRoyalEdge) error {
...
func (m *Manager) getTrackerForEdge(ctx context.Context, edge chainabstraction.VerifiedRoyalEdge) (*edgetracker.Tracker, error) {
	// Retry until you get the previous assertion Hash.
	assertionHash, err := retry.UntilSucceeds(ctx, func() (chainabstraction.AssertionHash, error) {
	...
	claimedHash := chainabstraction.AssertionHash{Hash: common.Hash(claimedAssertionId)}
	...
	var edgeTrackerAssertionInfo layer2stateprovider.AssociatedAssertionMetadata
	if !ok {
		assertionCreationInfo, creationErr := retry.UntilSucceeds(ctx, func() (*chainabstraction.AssertionCreatedInfo, error) {
			return m.chain.ReadAssertionCreationInfo(ctx, claimedHash)
		})
		if creationErr != nil {
			return nil, creationErr
		}
		prevCreationInfo, prevCreationErr := retry.UntilSucceeds(ctx, func() (*chainabstraction.AssertionCreatedInfo, error) {
		...
		fromState := chainabstraction.GoGlobalStateFromSolidity(assertionCreationInfo.BeforeState.GlobalState)
		edgeTrackerAssertionInfo = layer2stateprovider.AssociatedAssertionMetadata{
			FromState:            fromState,
			BatchLimit:           layer2stateprovider.Batch(prevCreationInfo.InboxMaxCount.Uint64()),
...
func (m *Manager) LatestConfirmedState(ctx context.Context) (chainabstraction.GoGlobalState, error) {
	...
	if err != nil {
		return chainabstraction.GoGlobalState{}, err
	}
	info, err := m.chain.ReadAssertionCreationInfo(ctx, latestConfirmed.Id())
	if err != nil {
		return chainabstraction.GoGlobalState{}, err
	}
	return chainabstraction.GoExecutionStateFromSolidity(info.AfterState).GlobalState, nil
}

func (m *Manager) LatestAgreedState(ctx context.Context) (chainabstraction.GoGlobalState, error) {
