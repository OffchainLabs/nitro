...
type CreatedValidatorFork struct {
	Leaf1              chainabstraction.Assertion
	Leaf2              chainabstraction.Assertion
	Chains             []*solimplementation.AssertionChain
...
func CreateTwoValidatorFork(
	...
	genesis, err := honestStateManager.ExecutionStateAfterPreviousState(ctx, 0, chainabstraction.GoGlobalState{})
	if err != nil {
		return nil, err
	}
	...
	genesis, err = evilStateManager.ExecutionStateAfterPreviousState(ctx, 0, chainabstraction.GoGlobalState{})
	if err != nil {
		return nil, err
	}
...
type ChainSetup struct {
	...
	autoDeposit                bool
	challengeTestingOpts       []challengetesting.Opt
...
func WithChallengeTestingOpts(opts ...challengetesting.Opt) Opt {
	return func(setup *ChainSetup) {
...
func ChainsWithEdgeChallengeManager(opts ...Opt) (*ChainSetup, error) {
	...
	if waitErr := challengetesting.WaitForTx(ctx, backend, tx); waitErr != nil {
	...
	if !setp.autoDeposit {
		...
		if waitErr := challengetesting.WaitForTx(ctx, backend, mintTx); waitErr != nil {
			return nil, errors.Wrap(waitErr, "errored waiting for transaction")
		}
	...
	if setp.EnableSafeFastConfirmation {
		...
		safeAddress, err = retry.UntilSucceeds(ctx, func() (common.Address, error) {
			...
			err = challengetesting.TxSucceeded(ctx, tx, safeAddress, backend, err)
			if err != nil {
				return common.Address{}, err
			}
		...
		safeProxyAddress, err = retry.UntilSucceeds(ctx, func() (common.Address, error) {
			...
			err = challengetesting.TxSucceeded(ctx, tx, safeProxyAddress, backend, err)
			if err != nil {
				return common.Address{}, err
			}
	...
	chains := make([]*solimplementation.AssertionChain, 0)
	for _, acc := range accs[1:] {
		...
		chain, chainErr := solimplementation.NewAssertionChain(
			ctx,
			addresses.Rollup,
			challengeManagerAddr,
			acc.TxOpts,
			backend,
			solimplementation.NewChainBackendTransactor(backend),
			assertionChainOpts...,
		)
		if chainErr != nil {
			return nil, chainErr
	...
	if !setp.autoDeposit {
		for i := 0; i < len(accs); i++ {
			...
			if waitErr := challengetesting.WaitForTx(ctx, backend, transferTx); waitErr != nil {
			...
			if waitErr := challengetesting.WaitForTx(ctx, backend, approveTx); waitErr != nil {
	...
	tx, err := retry.UntilSucceeds(ctx, func() (*types.Transaction, error) {
		creationTx, creationErr := rollupCreator.CreateRollup(
			...
			rollupgen.RollupCreatorRollupDeploymentParams{
				...
				MaxDataSize:               big.NewInt(challengetesting.MaxDataSize),
		...
		err = challengetesting.TxSucceeded(ctx, creationTx, rollupCreatorAddress, backend, err)
		if err != nil {
			return nil, err
		}
	...
	for addr, items := range txs {
		for _, item := range items {
			_, err = retry.UntilSucceeds(ctx, func() (*types.Transaction, error) {
				...
				if waitErr := challengetesting.WaitForTx(ctx, backend, innerTx); waitErr != nil {
...
func deployBridgeCreator(
	...
	if useMockBridge {
		...
		bridgeTemplate, err = retry.UntilSucceeds(ctx, func() (common.Address, error) {
			...
			err2 = challengetesting.TxSucceeded(ctx, tx, bridgeTemplateAddr, backend, err2)
			if err2 != nil {
	...
	if useBlobs {
		reader, err2 := retry.UntilSucceeds(ctx, func() (common.Address, error) {
			...
			err3 = challengetesting.TxSucceeded(ctx, tx, readerAddr, backend, err3)
			if err3 != nil {
	...
	seqInboxTemplate, err := retry.UntilSucceeds(ctx, func() (common.Address, error) {
		...
		err2 = challengetesting.TxSucceeded(ctx, tx, seqInboxTemplateAddr, backend, err2)
		if err2 != nil {
	...
	inboxTemplate, err := retry.UntilSucceeds(ctx, func() (common.Address, error) {
		...
		err2 = challengetesting.TxSucceeded(ctx, tx, inboxTemplateAddr, backend, err2)
		if err2 != nil {
	...
	rollupEventBridgeTemplate, err := retry.UntilSucceeds(ctx, func() (common.Address, error) {
		...
		err2 = challengetesting.TxSucceeded(ctx, tx, rollupEventBridgeTemplateAddr, backend, err2)
		if err2 != nil {
	...
	outboxTemplate, err := retry.UntilSucceeds(ctx, func() (common.Address, error) {
		...
		err2 = challengetesting.TxSucceeded(ctx, tx, outboxTemplateAddr, backend, err2)
		if err2 != nil {
	...
	result, err := retry.UntilSucceeds(ctx, func() (*bridgeCreationResult, error) {
		...
		err2 = challengetesting.TxSucceeded(ctx, tx, bridgeCreatorAddr, backend, err2)
		if err2 != nil {
			return nil, err2
		}
	...
	_, err = retry.UntilSucceeds(ctx, func() (*types.Transaction, error) {
		...
		err2 = challengetesting.TxSucceeded(ctx, tx, result.bridgeCreatorAddr, backend, err2)
		if err2 != nil {
			return nil, err2
		}
		return tx, nil
...
func deployRollupCreator(
	...
	rollupAdminLogic, err := retry.UntilSucceeds(ctx, func() (common.Address, error) {
		...
		err2 = challengetesting.TxSucceeded(ctx, tx, rollupAdminLogicAddr, backend, err2)
		if err2 != nil {
			return common.Address{}, err2
		}
	...
	rollupUserLogic, err := retry.UntilSucceeds(ctx, func() (common.Address, error) {
		...
		err2 = challengetesting.TxSucceeded(ctx, tx, rollupUserLogicAddr, backend, err2)
		if err2 != nil {
			return common.Address{}, err2
		}
	...
	result, err := retry.UntilSucceeds(ctx, func() (*creatorResult, error) {
		...
		err2 = challengetesting.TxSucceeded(ctx, tx, rollupCreatorAddress, backend, err2)
		if err2 != nil {
			return nil, err2
		}
	...
	validatorWalletCreator, err := retry.UntilSucceeds(ctx, func() (common.Address, error) {
		...
		err2 = challengetesting.TxSucceeded(ctx, tx, validatorWalletCreatorAddr, backend, err2)
		if err2 != nil {
			return common.Address{}, err2
		}
	...
	upgradeExecutor, err := retry.UntilSucceeds(ctx, func() (common.Address, error) {
		...
		err2 = challengetesting.TxSucceeded(ctx, tx, upgradeExecutorAddr, backend, err2)
		if err2 != nil {
			return common.Address{}, err2
		}
	...
	_, err = retry.UntilSucceeds(ctx, func() (*types.Transaction, error) {
		...
		if err2 := challengetesting.WaitForTx(ctx, backend, tx); err2 != nil {
			return nil, err2
		}
		return tx, nil
	})
