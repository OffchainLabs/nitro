...
var (
	...
	// ProofArgs for submission to the chainabstraction.
...
type L2StateBackend struct {
	layer2stateprovider.HistoryCommitmentProvider
	stateRoots              []common.Hash
	executionStates         []*chainabstraction.ExecutionState
	...
	numBigSteps             uint64
	numBatches              uint64
	challengeLeafHeights    []layer2stateprovider.Height
}
...
func NewWithMockedStateRoots(stateRoots []common.Hash, opts ...Opt) (*L2StateBackend, error) {
	...
	s := &L2StateBackend{
		...
		numBigSteps: 1,
		challengeLeafHeights: []layer2stateprovider.Height{
			challengetesting.LevelZeroBlockEdgeHeight,
			challengetesting.LevelZeroBigStepEdgeHeight,
			challengetesting.LevelZeroSmallStepEdgeHeight,
		},
	}
	for _, o := range opts {
		o(s)
	}
	commitmentProvider := layer2stateprovider.NewHistoryCommitmentProvider(s, s, s, s.challengeLeafHeights, s, nil)
...
func WithLayerZeroHeights(heights *chainabstraction.LayerZeroHeights, numBigSteps uint8) Opt {
	return func(s *L2StateBackend) {
		challengeLeafHeights := make([]layer2stateprovider.Height, 0)
		challengeLeafHeights = append(challengeLeafHeights, layer2stateprovider.Height(heights.BlockChallengeHeight))
		for i := uint8(0); i < numBigSteps; i++ {
			challengeLeafHeights = append(challengeLeafHeights, layer2stateprovider.Height(heights.BigStepChallengeHeight))
		}
		challengeLeafHeights = append(challengeLeafHeights, layer2stateprovider.Height(heights.SmallStepChallengeHeight))
...
func NewForSimpleMachine(
	...
	nextMachineState := &chainabstraction.ExecutionState{
	...
	for block := uint64(0); ; block++ {
		...
		if s.blockDivergenceHeight > 0 {
			...
			machHash = chainabstraction.ComputeSimpleMachineChallengeHash(state)
		}
...
func (s *L2StateBackend) UpdateAPIDatabase(database db.Database) {
	commitmentProvider := layer2stateprovider.NewHistoryCommitmentProvider(s, s, s, s.challengeLeafHeights, s, database)
	s.HistoryCommitmentProvider = *commitmentProvider
}

// ExecutionStateAfterPreviousState produces the l2 state to assert at the message number specified.
func (s *L2StateBackend) ExecutionStateAfterPreviousState(ctx context.Context, maxInboxCount uint64, previousGlobalState chainabstraction.GoGlobalState) (*chainabstraction.ExecutionState, error) {
...
func (s *L2StateBackend) maybeDivergeState(state *chainabstraction.ExecutionState, block uint64, step uint64) {
...
func (s *L2StateBackend) getMachineHash(machine Machine, block uint64) common.Hash {
	...
	return chainabstraction.ComputeSimpleMachineChallengeHash(state)
}
