...
var (
	_ layer2stateprovider.L2MessageStateCollector = (*L2StateBackend)(nil)
	_ layer2stateprovider.MachineHashCollector    = (*L2StateBackend)(nil)
)
	...
	t.Run("produces a block challenge commitment with height up to", func(t *testing.T) {
		got, err := provider.HistoryCommitment(
			...
			&layer2stateprovider.HistoryCommitmentRequest{
				AssertionMetadata:           simpleAssertionMetadata(),
				UpperChallengeOriginHeights: []layer2stateprovider.Height{},
				UpToHeight:                  option.Some(layer2stateprovider.Height(2)),
			},
		)
		require.NoError(t, err)
	...
	t.Run("produces a subchallenge history commitment with claims matching higher level start end leaves", func(t *testing.T) {
		blockChallengeCommit, err := provider.HistoryCommitment(
			...
			&layer2stateprovider.HistoryCommitmentRequest{
				AssertionMetadata:           simpleAssertionMetadata(),
				UpperChallengeOriginHeights: []layer2stateprovider.Height{},
				UpToHeight:                  option.Some(layer2stateprovider.Height(1)),
			},
		)
		require.NoError(t, err)
