...
func mockMachineAtBlock(_ context.Context, block uint64) (Machine, error) {
	...
	startState := &chainabstraction.ExecutionState{
...
func setupStates(t *testing.T, numStates, divergenceHeight uint64) ([]*chainabstraction.ExecutionState, []common.Hash) {
	t.Helper()
	states := make([]*chainabstraction.ExecutionState, numStates)
	...
	for i := uint64(0); i < numStates; i++ {
		...
		states[i] = state
		roots[i] = chainabstraction.ComputeSimpleMachineChallengeHash(state)
	}
	return states, roots
}

func newTestingMachine(
	assertionChainExecutionStates []*chainabstraction.ExecutionState,
	opts ...Opt,
	...
	for i := 0; i < len(stateRoots); i++ {
		...
		stateRoots[i] = chainabstraction.ComputeSimpleMachineChallengeHash(state)
	}
	s := &L2StateBackend{
