...
type Machine interface {
	...
	GetExecutionState() *chainabstraction.ExecutionState
	Hash() common.Hash
	IsStopped() bool
	Clone() Machine
...
type SimpleMachine struct {
	step           uint64
	state          *chainabstraction.ExecutionState
	maxBatchesRead *big.Int
}

func NewSimpleMachine(startingState *chainabstraction.ExecutionState, maxBatchesRead *big.Int) *SimpleMachine {
...
func (m *SimpleMachine) GetExecutionState() *chainabstraction.ExecutionState {
	stateCopy := *m.state
	return &stateCopy
}
...
func (m *SimpleMachine) IsStopped() bool {
	if m.step == 0 && m.state.MachineStatus == chainabstraction.MachineStatusFinished {
	...
	return m.state.MachineStatus != chainabstraction.MachineStatusRunning
}
...
func (m *SimpleMachine) Step(steps uint64) error {
	for ; steps > 0; steps-- {
		...
		m.state.MachineStatus = chainabstraction.MachineStatusRunning
		m.step++
		...
		if m.state.GlobalState.PosInBatch%stepsPerBatch == 0 {
			...
			m.state.MachineStatus = chainabstraction.MachineStatusFinished
		}
		if m.Hash()[0] == 0 {
			m.state.MachineStatus = chainabstraction.MachineStatusFinished
		}
...
func VerifySimpleMachineOneStepProof(beforeStateRoot common.Hash, claimedAfterStateRoot common.Hash, step uint64, maxBatchesRead *big.Int, proof []byte) bool {
	...
	state := &chainabstraction.ExecutionState{
		...
		MachineStatus: chainabstraction.MachineStatusRunning,
	}
	mach := NewSimpleMachine(state, maxBatchesRead)
	mach.step = step
	if step == 0 || mach.Hash()[0] == 0 {
		mach.state.MachineStatus = chainabstraction.MachineStatusFinished
	}
