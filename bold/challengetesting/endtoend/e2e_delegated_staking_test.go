...
import (
	...
	"github.com/offchainlabs/nitro/bold/challengetesting/endtoend/backend"
...
func TestEndToEnd_DelegatedStaking(t *testing.T) {
	...
	challengeTestingOpts := []challengetesting.Opt{
		challengetesting.WithConfirmPeriodBlocks(protocolCfg.challengePeriodBlocks),
		challengetesting.WithLayerZeroHeights(&protocolCfg.layerZeroHeights),
		challengetesting.WithNumBigStepLevels(protocolCfg.numBigStepLevels),
	}
	...
	honestChain := setupAssertionChain(
		...
		rollupAddr.Rollup,
		honestTxOpts,
		solimplementation.WithCustomWithdrawalAddress(fundsCustodianOpts.From),
	)
	...
	evilChain := setupAssertionChain(
		...
		rollupAddr.Rollup,
		evilTxOpts,
		solimplementation.WithCustomWithdrawalAddress(evilFundsCustodianOpts.From),
	)
	...
	t.Run("expects honest validator to win challenge", func(t *testing.T) {
		...
		for neutralCtx.Err() == nil && !confirmed {
			...
			require.NoError(t, err)
			for i.Next() {
				creationInfo, err2 := evilChain.ReadAssertionCreationInfo(evilCtx, chainabstraction.AssertionHash{Hash: i.Event.AssertionHash})
