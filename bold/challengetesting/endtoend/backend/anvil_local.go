...
func (a *AnvilLocal) Client() chainabstraction.ChainBackend {
	return a.client
}
...
func (a *AnvilLocal) DeployRollup(ctx context.Context, opts ...challengetesting.Opt) (*setup.RollupAddresses, error) {
	prod := false
	...
	if waitErr := challengetesting.WaitForTx(ctx, a.client, tx); waitErr != nil {
	...
	result, err := setup.DeployFullRollupStack(
		...
		challengetesting.GenerateRollupConfig(
			prod,
			wasmModuleRoot,
			rollupOwner,
			anvilLocalChainID,
	...
	if waitErr := challengetesting.WaitForTx(ctx, a.client, mintTx); waitErr != nil {
		return nil, errors.Wrap(waitErr, "errored waiting for transaction")
	}
	...
	for _, acc := range a.accounts[1:] {
		...
		if waitErr := challengetesting.WaitForTx(ctx, a.client, transferTx); waitErr != nil {
		...
		if waitErr := challengetesting.WaitForTx(ctx, a.client, approveTx); waitErr != nil {
