...
type expect func(t *testing.T, ctx context.Context, addresses *setup.RollupAddresses, be chainabstraction.ChainBackend, honestValidatorAddress common.Address) error
	...
	t.Run("honest essential edges confirmed by challenge win", func(t *testing.T) {
		...
		for ctx.Err() == nil && !confirmed {
			...
			for i.Next() {
				...
				if isChallengeParent && assertionNode.Status != uint8(chainabstraction.AssertionConfirmed) {
