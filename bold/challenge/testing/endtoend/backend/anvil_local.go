...
func (a *AnvilLocal) Start(ctx context.Context) error {
	...
	for time.Now().Before(deadline) {
		...
		if err == nil {
			...
			backend := backend.NewBackendWrapper(ethc, rpc.LatestBlockNumber)
...
func (a *AnvilLocal) DeployRollup(ctx context.Context, opts ...testing.Opt) (*setup.RollupAddresses, error) {
	prod := false
	...
	if waitErr := testing.WaitForTx(ctx, a.client, tx); waitErr != nil {
	...
	if waitErr := testing.WaitForTx(ctx, a.client, mintTx); waitErr != nil {
		return nil, errors.Wrap(waitErr, "errored waiting for transaction")
	}
	...
	for _, acc := range a.accounts[1:] {
		...
		if waitErr := testing.WaitForTx(ctx, a.client, transferTx); waitErr != nil {
		...
		if waitErr := testing.WaitForTx(ctx, a.client, approveTx); waitErr != nil {
