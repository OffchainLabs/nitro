...
- **Retryable Tickets**: 
    ...
    - Retryable ticket now aren't redeemed in the same block as when the `redeem` function is called. The user's transaction causes the retryable to be scheduled to be redeemed in a subsequent block.
