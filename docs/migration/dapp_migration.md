...
- **Lower contract size limit**: Contracts of up to 48KB were deployable, but now only up to 24KB as specified in [EIP 170](https://eips.ethereum.org/EIPS/eip-170). Previously deployed contracts above the limit will be maintained (as well as other contracts deployed by these legacy contracts)
- **Retryable Tickets**: 
