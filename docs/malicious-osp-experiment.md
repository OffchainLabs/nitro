# Malicious Validator Experiment: Forcing a BOLD Challenge to One-Step Proof (OSP)
...
- Keep **L1 contracts unchanged** so that the on-chain OSP implementation remains the ground truth and the malicious node **loses at OSP**.
