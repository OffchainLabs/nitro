...
RUN apt-get update && \
    ...
    # pinned rust 1.89.0
RUN curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y --default-toolchain 1.89.0 --target x86_64-unknown-linux-gnu,wasm32-unknown-unknown,wasm32-wasip1
COPY ./Makefile ./
...
FROM rust:1.89.0-slim-bookworm AS prover-header-builder
WORKDIR /workspace
...
FROM rust:1.89.0-slim-bookworm AS prover-builder
WORKDIR /workspace
RUN export DEBIAN_FRONTEND=noninteractive && \
    apt-get update && \
