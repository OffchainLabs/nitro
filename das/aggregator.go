...
func (a *Aggregator) Store(ctx context.Context, message []byte, timeout uint64, sig []byte) (*arbstate.DataAvailabilityCertificate, error) {
	...
	if successfullyStoredCount < a.requiredServicesForStore {
		return nil, fmt.Errorf("Aggregator failed to store message to at least %d out of %d DASes (assuming %d are honest), aggregate signer mask: %X, errors received %d, %v", a.requiredServicesForStore, len(a.services), a.config.AssumedHonest, aggSignersMask, storeFailures, errs)
	}
